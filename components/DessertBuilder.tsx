...
// The Dessert/Ingredients summary card's own fixed footprint (see its own
...
export function DessertBuilder({
  ...
  // padding, and the divider plus its own two dessert gaps -- split evenly,
  ...
  // Curated starter recipes (2026-08-09 on Salad/Smoothie, extended to
  // every other direct-ingredient builder 2026-08-14, this one included,
  // though no real curated dessert recipes exist in the reference database
  // yet -- listCuratedRecipes('dessert') below just returns an empty list
  // until some do, the same graceful "nothing to show" behavior this
  // section already has for a builder with zero recipes) -- see
  ...
  useEffect(() => {
    let isCurrent = true;
    listCuratedRecipes('dessert').then((recipes) => {
  ...
  async function finishDessert(finalIngredients: DessertIngredient[]) {
  ...
  function renderSummaryCard(squareBottom: boolean) {
    return (
      ...
      >
        ...
        >
          <Text style={[styles.summaryDessertName, { color: tabColor }]} numberOfLines={2}>
        ...
        <View style={styles.summaryRightColumn}>
          ...
          <View style={styles.summaryDoneRow}>
            {/* Edit mode: this card only ever shows while addingIngredient
                ...
                ingredients.length > 0 gate) -- a dessert being edited already
    ...
    return (
      <>
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          <TouchableOpacity style={[styles.primaryButton, { backgroundColor: tabColor }]} onPress={() => void finishDessert(ingredients)}>
    ...
    <>
      ...
      {!servingsConfirmed ? (
        <View style={[styles.formCard, { borderColor: tabColor }]}>
          ...
          {curatedRecipes.length > 0 && !editDessertId && !fromFavoriteId ? (
          ...
          <TouchableOpacity
            style={[
              ...
              dessertFormReady ? { backgroundColor: tabColor } : styles.primaryButtonMuted,
            ]}
        ...
        <>
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {editDessertId ? null : renderFavoriteToggle()}
            ...
            // which doesn't exist in this branch). finishDessert is called
            ...
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={styles.emptyText}>
                {(dessertName.trim() || 'Dessert')} ready with {ingredients.length} ingredient
              ...
              >
                <Text style={styles.primaryButtonText}>{editDessertId ? 'Save Changes' : 'Save & Finish Dessert'}</Text>
              </TouchableOpacity>
            </View>
          )}
        </>
...
const styles = StyleSheet.create({
  ...
  // Two columns (dessert info, scrollable ingredients) divided by a thin
  ...
  // 2026-07-28, explicitly requested -- dessert name pinned to the top of
  ...
  // site (was 1), explicitly requested so a longer dessert name wraps to a
