...
export function SmoothieBuilder({
  ...
  function confirmAndFinishSmoothie(finalIngredients: SmoothieIngredient[]) {
    ...
    if (flaggedFoods.length >= 2) {
      Alert.alert(
        ...
        `This smoothie combines ${flaggedFoods.length} raw goitrogenic vegetables (${flaggedFoods.join(', ')}). Blending them makes it easy to eat far more of these at once than you'd eat cooked and separate. Consider cooking one first, or using less of them.`,
        [
          { text: 'Go back and adjust', style: 'cancel' },
          { text: 'Continue anyway', onPress: () => void finishSmoothie(finalIngredients) },
        ],
      );
      return;
    }
