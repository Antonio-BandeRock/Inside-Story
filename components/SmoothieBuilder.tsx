...
// The final review screen's own real recipe-card-style ingredient line,
// 2026-08-17 -- "1 cup Spinach (chopped, blended)," matching how a real
// recipe actually reads an ingredient list rather than the shorter
// "name — amount" rows used elsewhere in this file.
function formatFinalIngredientText(ingredient: SmoothieIngredient): string {
...
export function SmoothieBuilder({
  ...
  // Whether the collapsible smoothie/ingredients card (point 2) is currently
  ...
  // whole add/edit/remove/complete flow) -- included in finishSmoothie's own
  ...
  // the unchanged SmoothieIngredient.prepNote/SmoothieIngredientInput.prepNote
  ...
  // identical real detail before Save & Finish Smoothie is ever tapped.
  ...
  // Whole-dish Steps, 2026-08-17 (point 6 of that day's own redesign) --
  // reused unchanged from SideBuilder.tsx/SaladBuilder.tsx's own identical
  // function. Shows both in the edit-mode overview and on the final
  // review screen (see renderStepsSection's own two call sites below).
  function renderStepsSection(labelled = true) {
    return (
      <StepsEditor
        ...
        placeholder="e.g., Blend on high for 60 seconds until smooth."
  ...
  // The collapsible smoothie/ingredients card's own real content, 2026-08-17
  // (points 2/7 of that day's own redesign) -- rendered inside
  // CollapsibleOverlayCard's own overlay (see that component and its two
  // real call sites below) rather than an always-visible two-column card.
    ...
    return (
      <>
        {/* 2026-08-17, direct report: "it already has the name of the side
            ...
            smoothieName as collapsedLabel right above this section. Serves/
            serving-size and the "tap to change smoothie details" link stay --
        ...
        >
          ...
          <Text style={[styles.secondaryButtonText, { color: tabColor, marginTop: 4 }]}>Tap to change smoothie details</Text>
        </TouchableOpacity>
        ...
        {/* Edit mode: this card only ever shows while addingIngredient is
            ...
            > 0 gate) -- a smoothie being edited already has ingredients by
      ...
      <>
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          {/* Real steps, edit mode -- 2026-08-17. Its own bordered card,
              ...
              saved smoothie to fix something is exactly as much "the area
  ...
  if (servingsConfirmed && finishStep === 'building' && !pendingResolved) {
    ...
    // Nothing chosen yet -- the collapsed smoothie/ingredients card (point 2)
    ...
    return (
      <View style={styles.pickerScreen}>
        <CollapsibleOverlayCard
          collapsedLabel={smoothieName.trim() || 'Smoothie'}
          tabColor={tabColor}
        ...
        <View style={[styles.formCard, { borderColor: tabColor }]}>
            ...
            <View style={styles.findRecipeSection}>
              ...
              >
                <Text style={styles.findRecipeLinkText} numberOfLines={1}>
                  My Recipes (your saved smoothies)
                </Text>
          ...
          <TouchableOpacity
            style={[
              ...
              smoothieFormReady ? { backgroundColor: colors.buttonColor } : styles.primaryButtonMuted,
            ]}
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {/* Real, per-ingredient Prep Steps, 2026-08-17 (point 6:
                  ...
                  Smoothie"/"Save Ingredient" time (see saveIngredient's own
              ...
              <View style={{ marginTop: 7 }}>
                <StepsEditor
                  ...
                  addAnotherLabel="+ Add Another Step"
                  completeLabel="Steps Complete"
                  placeholder="e.g., Peel and chop into chunks before blending"
                  scrollViewRef={scrollViewRef}
                />
              </View>
            ...
            // created for that smoothie, allowing them to see how it looks,
            ...
            // the extras nudge already satisfied/dismissed. confirmAndFinishSmoothie
            ...
            // 'building' for editSmoothieId), which is why editSmoothieId ?
            // 'Save Changes' below is a defensive fallback, not something
            // this branch is actually expected to render.
            <>
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={[styles.overviewSmoothieName, { color: tabColor }]}>{smoothieName.trim() || 'Smoothie'}</Text>
              <Text style={styles.summaryDetailText}>
              ...
              {/* Real, live-computed nutrition/condition preview, point 7 --
                  the exact same real detail a saved smoothie's own Digest My
                  Kitchen card shows, computed here BEFORE this smoothie is ever
                  saved (see nutritionHighlights/conditionNotes' own effect
                  above). Both silently render nothing when empty rather
                  than an empty box -- a smoothie with no real standout nutrient
            ...
            {nutritionHighlights.length > 0 ? (
              <View style={styles.bandOut}>
                <HomeSectionBand
                  title="What This Smoothie Gives You"
...
const styles = StyleSheet.create({
  ...
  // 2026-08-17 -- see the Smoothie Name field's own comment for why this
  ...
  // summaryRightColumn/summarySmoothieName lived here until 2026-08-17 -- the
  // old two-column summary card (smoothie info on the left, a scrollable
  ...
  juiceAdvisoryText: { ...typography.caption, ...textShadow },
  ...
  // collapsible smoothie/ingredients card above it, and pinned to whichever
  ...
  // separating the smoothie-info block from the Ingredients list beneath it --
