...
// The Snack/Ingredients summary card's own fixed footprint (see its own
...
export function SnackBuilder({
  ...
  // padding, and the divider plus its own two snack gaps -- split evenly,
  ...
  useEffect(() => {
    let isCurrent = true;
    listCuratedRecipes('snack').then((recipes) => {
  ...
  function renderSummaryCard(squareBottom: boolean) {
    return (
      ...
      >
        ...
        >
          <Text style={[styles.summarySnackName, { color: tabColor }]} numberOfLines={2}>
        ...
        <View style={styles.summaryRightColumn}>
          ...
          <View style={styles.summaryDoneRow}>
            {/* Edit mode: this card only ever shows while addingIngredient
                ...
                ingredients.length > 0 gate) -- a snack being edited already
    ...
    return (
      <>
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          <TouchableOpacity style={[styles.primaryButton, { backgroundColor: tabColor }]} onPress={() => void finishSnack(ingredients)}>
    ...
    <>
      ...
      {!servingsConfirmed ? (
        <View style={[styles.formCard, { borderColor: tabColor }]}>
          ...
          {curatedRecipes.length > 0 && !editSnackId && !fromFavoriteId ? (
          ...
          <TouchableOpacity
            style={[
              ...
              snackFormReady ? { backgroundColor: tabColor } : styles.primaryButtonMuted,
            ]}
        ...
        <>
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {editSnackId ? null : renderFavoriteToggle()}
            ...
            // which doesn't exist in this branch). finishSnack is called
            ...
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              >
                <Text style={styles.primaryButtonText}>{editSnackId ? 'Save Changes' : 'Save & Finish Snack'}</Text>
              </TouchableOpacity>
            </View>
          )}
        </>
...
const styles = StyleSheet.create({
  ...
  // Two columns (snack info, scrollable ingredients) divided by a thin
  ...
  // 2026-07-28, explicitly requested -- snack name pinned to the top of
  ...
  // site (was 1), explicitly requested so a longer snack name wraps to a
