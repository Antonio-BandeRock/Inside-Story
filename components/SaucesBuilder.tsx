...
// titleCaseSauceName below), matching standard title-case convention.
const SAUCE_NAME_MINOR_WORDS = new Set([
...
function titleCaseSauceName(text: string): string {
  ...
  return text.replace(/[A-Za-z']+/g, (word) => {
    ...
    if (!first && SAUCE_NAME_MINOR_WORDS.has(lower)) return lower;
...
// Reverses parseAmountValue for a saved sauce being reopened for editing --
...
// in the first place (shouldn't happen for anything SaucesBuilder itself
...
// Asked per INGREDIENT, not once for the whole sauce, 2026-07-29 -- a real
// sauce routinely combines ingredients that were each prepared
// completely differently (garlic added raw as a topping after the rest of
// the sauce is cooked; bacon pan-fried into bits in its own separate step
...
// sauce-wide answer was actively wrong for any sauce like that, not just
// imprecise. Required, not optional the way Sauce Name is, for the same
// nutrition reason.
//
...
// 'Reduced' added 2026-08-02 for this builder specifically -- the base list
// above is inherited from Side Builder verbatim, but reduction (simmering a
// liquid down to concentrate flavor and thicken it) is arguably THE
// defining sauce-making technique -- pan sauces, gravies, glazes are all
// built this way -- and the inherited list had no real option for it.
// Placed right after 'Boiled', the step reduction usually follows.
const COOKING_METHODS = [
  'N/A', 'Raw', 'Sautéed', 'Pan-Fried', 'Deep-Fried', 'Steamed', 'Boiled', 'Reduced', 'Baked', 'Roasted', 'Grilled', 'Stir-fried',
];
...
type SauceIngredient = {
  ...
  quantity: string;
  unit: string;
  // How THIS ingredient will be prepared for THIS sauce -- see
  // COOKING_METHODS' own comment for why this moved from a single
  // sauce-wide question to a per-ingredient one. cookingMethod is required
...
function formatFinalIngredientText(ingredient: SauceIngredient): string {
...
// Builds a single sauce from one or more ingredients -- Category ->
...
//
// The in-progress sauce (sauceName/servings/ingredients/etc.) is local
// component state only, same as before -- a sauce isn't a real, saved
// record until Save & Finish Sauce actually commits it (see finishSauce/
// saveSauce in lib/db.ts, 2026-08-01). Navigating away from a sauce mid-
// build still loses it; that's still a known, accepted gap, just a
// narrower one now that finishing a sauce for real is possible.
//
...
// ("Sauce Builder") was removed -- PageIdentityLabel's own bottom-
...
// place, once Sauce Name/Servings/Size is confirmed, that form splits into
// a compact two-column summary card (sauce info on the left, a scrollable
...
// base Fermentation/Beverage/Snack/Baked Goods/Soup Builder used -- neither
...
// still-accurate design reasoning, not a claim that SaucesBuilder went
// through that same history itself.
//
// Naming note: the lens key this builder is reached through
// ('saucesBuilder' in app/(tabs)/food.tsx's own FOOD_LENSES, already
// written before this pass started) is plural, unlike most of the other
// single-word builders' own lens keys -- so this component itself is named
// SaucesBuilder to match it exactly, but every internal identifier
// referring to ONE saved record (editSauceId, getSauce/saveSauce/
// updateSauce/deleteSauce, sauceName, the sauces/sauce_ingredients tables)
// stays singular "Sauce," the same pattern Salad/Smoothie/Fermentation/
// Beverage/Snack/Soup all already use -- only the outer component name
// bends to match the plural lens key.
//
// The oil/seasoning nudge is kept unchanged (same reasoning as Snack/Baked
// Goods/Soup -- vinaigrettes, mayo-based sauces, and herb/salt seasoning
// are all genuinely common). One concrete addition: 'Reduced' added to this
// builder's own local COOKING_METHODS list (see that constant's own
// comment), the same "add the one real missing prep option" move
// Fermentation made for 'Fermented' and Soup made for 'Simmered'.
export function SaucesBuilder({
  tabColor,
  // Set when reached via the Edit button on an already-saved sauce (see
  // app/food-items.tsx) -- 2026-08-01. Loads that sauce's real data into
  ...
  // machinery a fresh sauce already uses. finishSauce below branches on this
  // to call updateSauce (in place) instead of saveSauce (a new row), and to
  // navigate back to the list afterward instead of resetting to blank --
  // "I fixed this sauce" should return you to where you came from, not
  // drop you into building a different one.
  editSauceId,
  // Set when reached via "Use this Favorite" (see app/food-items.tsx) --
  // 2026-08-08. Same shape as editSauceId's own prefill just below, except
  // this never marks anything as an edit -- finishSauce always creates a
  // genuinely NEW sauce from a favorite.
  fromFavoriteId,
  ...
  // field itself, or Change Food/Add to Sauce below it) could end up stuck
  ...
  // falls back to a plain "Sauce" in the summary/ingredient views
  // (sauceName.trim() || 'Sauce') as a harmless extra safety net, even
  ...
  // here (`(text) => setSauceName(titleCaseSauceName(text))`, passed directly
  ...
  // renders (titleCaseSauceName is a plain module-level function, setSauceName
  // is already a stable state setter), breaking the loop.
  const handleSauceNameChange = useCallback((text: string) => {
    setSauceName(titleCaseSauceName(text));
  }, []);
  ...
  // made). Explicitly corrected: every field in this whole form -- Sauce
  ...
  // below) -- true only once Sauce Name and all three pickers are actually
  // filled in/chosen.
  const sauceFormReady = !!sauceName.trim() && !!servings && !!servingSizeAmount && !!servingSizeUnit;
  ...
  // Whether the collapsible sauce/ingredients card (point 2) is currently
  ...
  // whole add/edit/remove/complete flow) -- included in finishSauce's own
  ...
  const [ingredients, setIngredients] = useState<SauceIngredient[]>([]);
  ...
  // Carried onto the SauceIngredient at add time rather than re-fetched
  ...
  // servingSizeUnit above. "Add to Sauce" below stays disabled until
  ...
  // the unchanged SauceIngredient.prepNote/SauceIngredientInput.prepNote
  ...
  // NEXT ingredient after "Add to Sauce," both land back on that same
  // category instead of a blank Category list -- a sauce very often
  ...
  // Loads an existing sauce's real data in place of the blank-builder
  // defaults above, 2026-08-01 -- runs once per editSauceId. sauce_ingredients
  ...
  useEffect(() => {
    if (!editSauceId) return;
    let isCurrent = true;

    (async () => {
      const sauce = await getSauce(editSauceId);
      if (!sauce || !isCurrent) return;

      const details = await getSauceIngredients(editSauceId);
      const loaded: SauceIngredient[] = [];
      ...
      setServings(formatAmountForPicker(sauce.servings, SERVINGS_PICKER_VALUES));
      setServingSizeAmount(formatAmountForPicker(sauce.servingSizeAmount, AMOUNT_PICKER_VALUES));
      setServingSizeUnit(sauce.servingSizeUnit);
  ...
  // Same shape as editSauceId's own effect just above -- see
  // SideBuilder.tsx's own identical fromFavoriteId effect for the full
  // reasoning. Never sets editSauceId-equivalent state, so finishSauce
  // below always creates a new sauce.
  useEffect(() => {
  ...
  useEffect(() => {
    if (openRecipeId && !editSauceId && !fromFavoriteId) {
  ...
  // below) -- that Quantity/Add-to-Sauce card's own Quantity field
  ...
  // the same instant. Without scrolling, "Add to Sauce" -- the very last
  ...
  // Sauce does nothing," which is exactly what a covered button looks like.
  ...
  // at all, 2026-07-29 -- it's asked per ingredient now, at "Add to Sauce"
  // time (see SauceIngredient's own comment).
  ...
  // Edit mode only (see editSauceId's own comment) -- whether the person has
  ...
  // Starts false so opening an existing sauce for editing always lands on
  // the ingredient overview first, not a "pick a Category" prompt --
  // explicitly requested, since re-opening a sauce is almost always to
  ...
  // flagged, which is wrong for genuinely oil-free/seasoning-free sauces
  ...
  // identical real detail before Save & Finish Sauce is ever tapped.
  ...
  // 2026-07-28 -- whichever AppTextInput was last focused (Sauce Name,
  ...
  // order they appear on screen (Sauce Name, then Servings, then Serving
  ...
  // 2026-07-28, including the exact wording for Sauce Name/Servings; the
  // other two follow the same "Please choose/enter..." phrasing.
  function handleContinuePress() {
    if (!sauceName.trim()) {
      showInfoAlert('Almost there', 'Please enter a name for this sauce.');
      return;
    }
    if (!servings) {
      showInfoAlert('Almost there', 'Please choose the # of Servings this sauce will contain.');
      return;
    }
    if (!servingSizeAmount) {
      showInfoAlert('Almost there', 'Please choose the Serving Size for this sauce.');
      return;
    }
    if (!servingSizeUnit) {
      showInfoAlert('Almost there', 'Please choose the Units for this sauce.');
      return;
    }
    dismissKeyboard();
    setServingsConfirmed(true);
  ...
  // THEN resets the builder back to a blank sauce, so a save failure never
  // silently loses what was just built -- the person sees a real error and
  // keeps their in-progress sauce to retry with, rather than it vanishing
  // either way.
  //
  // Shared by finishSauce and handlePreviewReport below -- see
  // SideBuilder.tsx's own identical helper for the full reasoning.
  function buildDepthIngredients(finalIngredients: SauceIngredient[]): MealIngredientInput[] {
  ...
  // needs to save the whole sauce -- the caller builds the true final list
  ...
  async function finishSauce(finalIngredients: SauceIngredient[], precomputedDepth?: RecipeDepthResult) {
    ...
    const ingredientInputs: SauceIngredientInput[] = finalIngredients.map((ingredient) => ({
    ...
    const payload = {
      ...
      // what was explicitly saved counts" rule Save & Finish Sauce
    ...
    try {
      if (editSauceId) {
        await updateSauce(editSauceId, payload);
      } else {
        await saveSauce(payload);
      }
    } catch (error) {
    ...
    if (alsoSaveAsFavorite) {
      try {
        await saveBuilderFavorite('sauce', payload);
      } catch (error) {
        console.error('[SaucesBuilder] Failed to save favorite', error);
        showInfoAlert('Sauce saved, favorite failed', `${finishedName} is saved, but saving it as a favorite didn't work. You can try favoriting it again later.`);
      }
    }

    // Editing an already-saved sauce returns to wherever it was opened from
    // (see app/food-items.tsx's Edit button, which pushed this screen) --
    // "I fixed this sauce" should go back to the list, not drop the person
    ...
    // Back to a blank sauce. From here the Lens Button starts a
    ...
    showInfoAlert('Sauce saved', `${finishedName} is saved. Starting a fresh sauce now.`);
  }
  ...
  async function handlePreviewReport() {
    ...
    try {
      ...
      console.error('[SaucesBuilder] Failed to compute the depth report', error);
      showInfoAlert('Report failed', 'Something went wrong building the report. You can still save this sauce directly.');
    } finally {
  ...
  // or ends the sauce entirely -- 2026-07-31, the two Save buttons.
  //
  //   'add-new' -> back to picking a Food Category, then a food, and round
  //                again, indefinitely, until the person chooses 'finish'.
  //   'finish'  -> saves the whole sauce (see finishSauce above) and resets
  //                the builder to a blank new sauce.
  async function saveIngredient(then: 'add-new' | 'finish') {
    if (!pendingResolved || !quantity || !unit || !ingredientCutPrep || !ingredientCookingMethod) {
      ...
      // appear on screen -- same approach as the sauce form's own Continue.
      const missing = [
        !quantity && 'Quantity',
        !unit && 'Units',
    ...
    const scoresForIngredient = reconciled.changed
      ...
      : pendingScores;
    const newIngredient: SauceIngredient = {
    ...
    if (then === 'finish') {
      void finishSauce(allIngredients);
    }
  }
  ...
  // sauce (the small X row-remove buttons, packed close together in this
  ...
  function renderStepsSection(labelled = true) {
    return (
      <StepsEditor
        ...
        placeholder="e.g., Sauté the aromatics first, then whisk in the roux and simmer until it thickens."
  ...
  // The Sauce/Ingredients summary card -- sauce name/servings/size on the
  ...
  function renderLabeledPicker(
    ...
    // the sauce-naming screen's own three fields (#-of-Servings/Serving
  ...
  // The collapsible sauce/ingredients card's own content, 2026-08-17 (point
  ...
  // real "review the whole sauce so far" moments. Full-width, single
    ...
    return (
      <>
        {/* 2026-08-17, direct report: "it already has the name of the side
            ...
            sauceName as collapsedLabel right above this section. Serves/
            serving-size and the "tap to change sauce details" link stay --
        ...
        >
          ...
          <Text style={[styles.secondaryButtonText, { color: tabColor, marginTop: 4 }]}>Tap to change sauce details</Text>
        </TouchableOpacity>
        ...
        {/* Edit mode: this card only ever shows while addingIngredient is
            ...
            > 0 gate) -- a sauce being edited already has ingredients by
  ...
  // (the sauce-name row only shows while !servingsConfirmed; the ingredient
  ...
  const sauceFormFields = useReorderedLabeledFields([
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          <RecipeDepthReport
            ...
            onSave={() => {
              ...
              void finishSauce(ingredients, reportData).finally(() => setSavingFromReport(false));
            }}
          />
        </ScrollView>
        <AppActionSheet
            ...
            ...(stagePickerFor ? getConditionStagingModel(stagePickerFor.code)?.stages ?? [] : []).map((stage) => ({
              ...
              onPress: () => {
                ...
                setConditionStage(code, stage.code)
                  ...
                  .catch((error) => {
                    console.error('[SaucesBuilder] Failed to save the declared healing stage', error);
  ...
  // requested: reopening an already-saved sauce to fix something shouldn't
  // assume the next thing wanted is picking a whole new Category. Landing
  // here by default (addingIngredient starts false) shows the sauce info
  ...
  // ahead of only when editSauceId is set).
  if (editSauceId && servingsConfirmed && !pendingResolved && !addingIngredient) {
    return (
      <>
        {infoAlertElement}
        {confirmSheetElement}
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          >
            <Text style={[styles.overviewSauceName, { color: tabColor }]} numberOfLines={2}>
              {sauceName.trim() || 'Sauce'}
            </Text>
            <Text style={styles.summaryDetailText}>Serves {servings || '?'}</Text>
            <Text style={styles.summaryDetailText}>
              {servingSizeAmount || '?'} {servingSizeUnit ?? '?'} / serving
          ...
          {/* Real steps, edit mode -- 2026-08-17. Its own bordered card,
              ...
              saved sauce to fix something is exactly as much "the area that
  ...
  // Continue, and again right after each "Add to Sauce") -- no separate
  ...
  if (servingsConfirmed && finishStep === 'building' && !pendingResolved) {
      ...
      return (
        <View style={styles.pickerScreen}>
          ...
          <FoodLookup
            ...
            allowedCategories={SAUCES_BUILDER_CATEGORIES}
    ...
    // Nothing chosen yet -- the collapsed sauce/ingredients card (point 2)
    ...
    return (
      <View style={styles.pickerScreen}>
        <CollapsibleOverlayCard
          collapsedLabel={sauceName.trim() || 'Sauce'}
          tabColor={tabColor}
      ...
      {!servingsConfirmed ? (
        <View style={[styles.formCard, { borderColor: tabColor }]}>
          ...
          {/* 2026-08-16 -- see SideBuilder.tsx's own identical comment for
              ...
              inline "Or Start From a Recipe" cards are gone, replaced by
              the link out to Food's own recipe lenses above. */}
          ...
          <View style={[styles.dishNameFieldWrap, { backgroundColor: inputBackground(tabColor) }]}>
            {NAVIGATION_HAND === 'left' ? (
              <>
                <VoiceInputButton onResult={(transcript) => handleSauceNameChange(transcript)} size={22} />
                <AppTextInput
                  style={[styles.formInput, styles.dishNameInputEmbedded]}
                  value={sauceName}
                  onChangeText={handleSauceNameChange}
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {/* Header, 2026-07-31 -- "Preparation Steps for X", title-cased.
                  ...
                  correct them). titleCaseSauceName is the same helper the
                  Sauce Name field uses, so the two can't drift apart. */}
              <View style={styles.pendingFoodRow}>
                ...
                <Text style={styles.pendingHeader} numberOfLines={2}>
                  Preparation Steps for {titleCaseSauceName(foodSummary(pendingResolved))}
                </Text>
              ...
              {/* Real, per-ingredient Prep Steps, 2026-08-17 (point 6:
                  ...
                  Sauce"/"Save Ingredient" time (see saveIngredient's own
              ...
              <View style={{ marginTop: 7 }}>
                <StepsEditor
                  ...
                  addAnotherLabel="+ Add Another Step"
                  completeLabel="Steps Complete"
                  placeholder="e.g., Simmer down until slightly thickened, whisk in the butter off the heat"
                  scrollViewRef={scrollViewRef}
                />
              </View>
            ...
            // nudgeDismissed's own comment above): plenty of real sauces
            ...
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={styles.emptyText}>
                No {missingExtras.map((extra) => extra.label).join(' or ')} logged for this sauce yet.
              </Text>
              <View style={styles.buttonRow}>
                <TouchableOpacity
                  style={styles.secondaryButton}
            ...
            // created for that sauce, allowing them to see how it looks,
            ...
            // the extras nudge already satisfied/dismissed. finishSauce is
            ...
            // 'building' for editSauceId), which is why editSauceId ?
            // 'Save Changes' below is a defensive fallback, not something
            // this branch is actually expected to render.
            <>
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={[styles.overviewSauceName, { color: tabColor }]}>{sauceName.trim() || 'Sauce'}</Text>
              <Text style={styles.summaryDetailText}>
              ...
              {/* Real, live-computed nutrition/condition preview, point 7 --
                  the exact same real detail a saved sauce's own Digest My
                  Kitchen card shows, computed here BEFORE this sauce is ever
                  saved (see nutritionHighlights/conditionNotes' own effect
                  above). Both silently render nothing when empty rather
                  than an empty box -- a sauce with no real standout nutrient
            ...
            {nutritionHighlights.length > 0 ? (
              <View style={styles.bandOut}>
                <HomeSectionBand
                  title="What This Sauce Gives You"
...
const styles = StyleSheet.create({
  ...
  // 2026-08-17 -- see the Sauce Name field's own comment for why this
  ...
  // Extra room above the Sauce Name form's own Continue button, 2026-07-28
  ...
  // summaryRightColumn/summarySauceName lived here until 2026-08-17 -- the
  // old two-column summary card (sauce info on the left, a scrollable
  ...
  // of summarySauceName/summaryIngredientRow/summaryRemoveButton above,
  ...
  // collapsible sauce/ingredients card above it, and pinned to whichever
  ...
  // separating the sauce-info block from the Ingredients list beneath it --
