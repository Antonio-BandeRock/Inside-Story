...
// titleCaseHandheldName below), matching standard title-case convention.
const HANDHELD_NAME_MINOR_WORDS = new Set([
...
function titleCaseHandheldName(text: string): string {
  ...
  return text.replace(/[A-Za-z']+/g, (word) => {
    ...
    if (!first && HANDHELD_NAME_MINOR_WORDS.has(lower)) return lower;
...
// Reverses parseAmountValue for a saved handheld being reopened for editing --
...
// in the first place (shouldn't happen for anything HandheldsBuilder itself
...
// Asked per INGREDIENT, not once for the whole handheld, 2026-07-29 -- a real
// handheld routinely combines ingredients that were each prepared
// completely differently (garlic added raw as a topping after the rest of
// the handheld is cooked; bacon pan-fried into bits in its own separate step
...
// handheld-wide answer was actively wrong for any handheld like that, not just
// imprecise. Required, not optional the way Handheld Name is, for the same
// nutrition reason.
//
...
// 'Toasted' added 2026-08-04 for this builder specifically -- the base list
// above is inherited from Side Builder verbatim, but toasting (dry-heating
// bread, a bun, or a tortilla until it browns and crisps) is one of the
// single most common things done to a Handhelds outer casing, and the
// inherited list had no real option for it -- 'Baked'/'Roasted' both imply
// something closer to fully cooking raw dough, not briefly crisping an
// already-baked casing. Placed right after 'Baked', the other dry-heat
// technique it's closest to.
const COOKING_METHODS = [
  'N/A', 'Raw', 'Sautéed', 'Pan-Fried', 'Deep-Fried', 'Steamed', 'Boiled', 'Baked', 'Toasted', 'Roasted', 'Grilled', 'Stir-fried',
];
...
type HandheldIngredient = {
  resolved: ResolvedFoodSelection;
  quantity: string;
  unit: string;
  // How THIS ingredient will be prepared for THIS handheld -- see
  // COOKING_METHODS' own comment for why this moved from a single
  // handheld-wide question to a per-ingredient one. cookingMethod is required
...
function formatFinalIngredientText(ingredient: HandheldIngredient): string {
...
// Builds a single handheld from one or more ingredients -- Category ->
...
//
// The in-progress handheld (handheldName/servings/ingredients/etc.) is local
// component state only, same as before -- a handheld isn't a real, saved
// record until Save & Finish Handheld actually commits it (see finishHandheld/
// saveHandheld in lib/db.ts, 2026-08-01). Navigating away from a handheld mid-
// build still loses it; that's still a known, accepted gap, just a
// narrower one now that finishing a handheld for real is possible.
//
...
// ("Handheld Builder") was removed -- PageIdentityLabel's own bottom-
...
// place, once Handheld Name/Servings/Size is confirmed, that form splits into
// a compact two-column summary card (handheld info on the left, a scrollable
...
// Created 2026-08-04 by directly adapting SideBuilder.tsx (the Food tab's
...
// base Fermentation/Beverage/Snack/Baked Goods/Soup/Sauces Builder used --
// neither Salad's nor Smoothie's own raw-goitrogenic-load warning applies
// here. The eleventh builder, and the first genuinely new one since Meal:
// requested after a Google-search-sourced builder-consolidation proposal
// named a real gap (sandwiches, wraps, burgers, and tacos all share the
// same four-step "outer casing -> primary protein -> toppings ->
// condiment/spread" assembly logic, and none of the other ten builders
// covered it). Deliberately does NOT add a new "layer role" tag per
// ingredient -- every other builder's flat, unordered ingredient list
// (category -> type -> food -> prep -> quantity/unit) already covers this
// fine in practice, since a person naturally adds bread, then protein,
// then toppings, then condiment in that order anyway; inventing a new
// per-ingredient "layer" UI field would be new interaction design no other
// builder has, not a proven pattern to reuse, and wasn't asked for. The
...
// still-accurate design reasoning, not a claim that HandheldsBuilder went
// through that same history itself.
//
// Naming note: "Layers" was the working name during the builder-
// consolidation discussion, renamed to "Handhelds" before any code was
// written -- every other builder is named after a recognizable food
// category (Soup, Snack, Sauces), not a structural description, and
// "Handhelds" is itself a real, already-used food-industry/menu category
// term that genuinely covers sandwich/wrap/burger/taco/burrito as one
// thing, the same way "Sides" or "Snacks" does. The lens key this builder
// is reached through ('handheldsBuilder' in app/(tabs)/food.tsx's own
// FOOD_LENSES) is plural, unlike most of the other single-word builders'
// own lens keys -- so this component itself is named HandheldsBuilder to
// match it exactly, but every internal identifier referring to ONE saved
// record (editHandheldId, getHandheld/saveHandheld/updateHandheld/
// deleteHandheld, handheldName, the handhelds/handheld_ingredients tables)
// stays singular "Handheld," the same pattern Sauces already established
// for exactly this same plural-lens-key situation.
//
// The oil/seasoning nudge is kept unchanged (same reasoning as Snack/Baked
// Goods/Soup/Sauces -- mayo, mustard, dressing-style condiments, and herb/
// salt seasoning are all genuinely common on a sandwich or taco). One
// concrete addition: 'Toasted' added to this builder's own local
// COOKING_METHODS list (see that constant's own comment) -- the base list
// inherited from Side Builder had no real option for the single most
// common thing done to a Handhelds outer casing, the same "add the one
// real missing prep option" move Fermentation made for 'Fermented', Soup
// made for 'Simmered', and Sauces made for 'Reduced'.
export function HandheldsBuilder({
  tabColor,
  // Set when reached via the Edit button on an already-saved handheld (see
  // app/food-items.tsx) -- 2026-08-01. Loads that handheld's real data into
  ...
  // machinery a fresh handheld already uses. finishHandheld below branches on this
  // to call updateHandheld (in place) instead of saveHandheld (a new row), and to
  ...
  // field itself, or Change Food/Add to Handheld below it) could end up stuck
  ...
  // falls back to a plain "Handheld" in the summary/ingredient views
  // (handheldName.trim() || 'Handheld') as a harmless extra safety net, even
  // though Continue itself should never actually let a blank one through
  // now.
  const [handheldName, setHandheldName] = useState('');
  // A stable function identity, 2026-07-30 -- a fresh inline arrow function
  // here (`(text) => setHandheldName(titleCaseHandheldName(text))`, passed directly
  ...
  // renders (titleCaseHandheldName is a plain module-level function, setHandheldName
  // is already a stable state setter), breaking the loop.
  const handleHandheldNameChange = useCallback((text: string) => {
    setHandheldName(titleCaseHandheldName(text));
  }, []);
  ...
  // made). Explicitly corrected: every field in this whole form -- Handheld
  ...
  // below) -- true only once Handheld Name and all three pickers are actually
  // filled in/chosen.
  const handheldFormReady = !!handheldName.trim() && !!servings && !!servingSizeAmount && !!servingSizeUnit;
  ...
  // Whether the collapsible handheld/ingredients card (point 2) is currently
  ...
  // whole add/edit/remove/complete flow) -- included in finishHandheld's own
  ...
  // Carried onto the HandheldIngredient at add time rather than re-fetched
  ...
  // servingSizeUnit above. "Add to Handheld" below stays disabled until
  ...
  // HandheldIngredient's own comment for why this replaced a single
  // handheld-wide question. Reset after each "Add to Handheld," same as
  ...
  // Prep Notes field with the same real ordered-list shape the whole-
  // handheld Steps feature already uses -- see StepsEditor.tsx. Serialized
  // to/from the unchanged HandheldIngredient.prepNote/
  // HandheldIngredientInput.prepNote string field at save/load time
  // (join('\n')), so no database schema change was needed for this.
  ...
  // NEXT ingredient after "Add to Handheld," both land back on that same
  // category instead of a blank Category list -- a handheld very often
  ...
  // Loads an existing handheld's real data in place of the blank-builder
  // defaults above, 2026-08-01 -- runs once per editHandheldId. handheld_ingredients
  ...
  useEffect(() => {
    if (!editHandheldId) return;
    let isCurrent = true;

    (async () => {
      const handheld = await getHandheld(editHandheldId);
      if (!handheld || !isCurrent) return;

      const details = await getHandheldIngredients(editHandheldId);
      ...
      if (!isCurrent) return;
      setHandheldName(handheld.name);
      setServings(formatAmountForPicker(handheld.servings, SERVINGS_PICKER_VALUES));
      setServingSizeAmount(formatAmountForPicker(handheld.servingSizeAmount, AMOUNT_PICKER_VALUES));
      setServingSizeUnit(handheld.servingSizeUnit);
  ...
  useEffect(() => {
    if (openRecipeId && !editHandheldId && !fromFavoriteId) {
  ...
  // below) -- that Quantity/Add-to-Handheld card's own Quantity field
  ...
  // the same instant. Without scrolling, "Add to Handheld" -- the very last
  ...
  // Handheld does nothing," which is exactly what a covered button looks like.
  ...
  // at all, 2026-07-29 -- it's asked per ingredient now, at "Add to Handheld"
  // time (see HandheldIngredient's own comment).
  ...
  // Edit mode only (see editHandheldId's own comment) -- whether the person has
  ...
  // Starts false so opening an existing handheld for editing always lands on
  // the ingredient overview first, not a "pick a Category" prompt --
  // explicitly requested, since re-opening a handheld is almost always to
  ...
  // flagged, which is wrong for genuinely oil-free/seasoning-free handhelds
  ...
  // identical real detail before Save & Finish Handheld is ever tapped.
  ...
  // 2026-07-28 -- whichever AppTextInput was last focused (Handheld Name,
  ...
  // order they appear on screen (Handheld Name, then Servings, then Serving
  ...
  // 2026-07-28, including the exact wording for Handheld Name/Servings; the
  // other two follow the same "Please choose/enter..." phrasing.
  function handleContinuePress() {
    if (!handheldName.trim()) {
      showInfoAlert('Almost there', 'Please enter a name for this handheld.');
      return;
    }
    if (!servings) {
      showInfoAlert('Almost there', 'Please choose the # of Servings this handheld will contain.');
      return;
    }
    if (!servingSizeAmount) {
      showInfoAlert('Almost there', 'Please choose the Serving Size for this handheld.');
      return;
    }
    if (!servingSizeUnit) {
      showInfoAlert('Almost there', 'Please choose the Units for this handheld.');
      return;
    }
    dismissKeyboard();
    setServingsConfirmed(true);
  ...
  // Persists the finished handheld (see saveHandheld/the handhelds/handheld_ingredients
  ...
  // THEN resets the builder back to a blank handheld, so a save failure never
  // silently loses what was just built -- the person sees a real error and
  // keeps their in-progress handheld to retry with, rather than it vanishing
  // either way.
  //
  // Shared by finishHandheld and handlePreviewReport below -- see
  // SideBuilder.tsx's own identical helper for the full reasoning.
  function buildDepthIngredients(finalIngredients: HandheldIngredient[]): MealIngredientInput[] {
  ...
  // needs to save the whole handheld -- the caller builds the true final list
  ...
  async function finishHandheld(finalIngredients: HandheldIngredient[], precomputedDepth?: RecipeDepthResult) {
    ...
    const ingredientInputs: HandheldIngredientInput[] = finalIngredients.map((ingredient) => ({
    ...
    const finishedName = handheldName.trim() || 'Handheld';
    const payload = {
      name: finishedName,
      ...
      // what was explicitly saved counts" rule Save & Finish Handheld
    ...
    try {
      if (editHandheldId) {
        await updateHandheld(editHandheldId, payload);
      } else {
        await saveHandheld(payload);
      }
    } catch (error) {
      console.error('[HandheldsBuilder] Failed to save handheld', error);
      showInfoAlert('Save failed', 'Something went wrong saving this handheld. Your ingredients are still here; please try again.');
      return;
    }
    ...
    // Back to a blank handheld. From here the Lens Button starts a
  ...
  async function handlePreviewReport() {
    ...
    try {
      ...
      console.error('[HandheldsBuilder] Failed to compute the depth report', error);
      showInfoAlert('Report failed', 'Something went wrong building the report. You can still save this handheld directly.');
    } finally {
  ...
  // or ends the handheld entirely -- 2026-07-31, the two Save buttons.
  //
  //   'add-new' -> back to picking a Food Category, then a food, and round
  //                again, indefinitely, until the person chooses 'finish'.
  //   'finish'  -> saves the whole handheld (see finishHandheld above) and resets
  //                the builder to a blank new handheld.
  async function saveIngredient(then: 'add-new' | 'finish') {
    if (!pendingResolved || !quantity || !unit || !ingredientCutPrep || !ingredientCookingMethod) {
      ...
      // appear on screen -- same approach as the handheld form's own Continue.
      const missing = [
        !quantity && 'Quantity',
        !unit && 'Units',
    ...
    const scoresForIngredient = reconciled.changed
      ...
      : pendingScores;
    const newIngredient: HandheldIngredient = {
    ...
    if (then === 'finish') {
      void finishHandheld(allIngredients);
    }
  }
  ...
  // handheld (the small X row-remove buttons, packed close together in this
  ...
  function renderStepsSection(labelled = true) {
    return (
      <StepsEditor
        ...
        placeholder="e.g., Toast the bun, layer the toppings, then add the sauce and close it up."
  ...
  // The Handheld/Ingredients summary card -- handheld name/servings/size on the
  ...
  function renderLabeledPicker(
    ...
    // the handheld-naming screen's own three fields (#-of-Servings/Serving
  ...
  // The collapsible handheld/ingredients card's own content, 2026-08-17
  ...
  // both are real "review the whole handheld so far" moments. Full-width,
    ...
    return (
      <>
        {/* 2026-08-17, direct report: "it already has the name of the side
            ...
            handheldName as collapsedLabel right above this section. Serves/
            serving-size and the "tap to change handheld details" link stay
            -- neither is shown anywhere else, so they're real, non-
        ...
        >
          ...
          <Text style={[styles.secondaryButtonText, { color: tabColor, marginTop: 4 }]}>Tap to change handheld details</Text>
        </TouchableOpacity>
        ...
        {/* Edit mode: this card only ever shows while addingIngredient is
            ...
            > 0 gate) -- a handheld being edited already has ingredients by
  ...
  // (the handheld-name row only shows while !servingsConfirmed; the ingredient
  ...
  const handheldFormFields = useReorderedLabeledFields([
    ...
    return (
      <>
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          <RecipeDepthReport
            dishName={handheldName.trim() || 'Handheld'}
            ...
            onSave={() => {
              ...
              void finishHandheld(ingredients, reportData).finally(() => setSavingFromReport(false));
            }}
          />
        </ScrollView>
        <AppActionSheet
            ...
            ...(stagePickerFor ? getConditionStagingModel(stagePickerFor.code)?.stages ?? [] : []).map((stage) => ({
              ...
              onPress: () => {
                ...
                setConditionStage(code, stage.code)
                  ...
                  .catch((error) => {
                    console.error('[HandheldsBuilder] Failed to save the declared healing stage', error);
  ...
  // requested: reopening an already-saved handheld to fix something shouldn't
  // assume the next thing wanted is picking a whole new Category. Landing
  // here by default (addingIngredient starts false) shows the handheld info
  ...
  // ahead of only when editHandheldId is set).
  if (editHandheldId && servingsConfirmed && !pendingResolved && !addingIngredient) {
    return (
      <>
        {infoAlertElement}
        {confirmSheetElement}
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          >
            <Text style={[styles.overviewHandheldName, { color: tabColor }]} numberOfLines={2}>
              {handheldName.trim() || 'Handheld'}
            </Text>
            <Text style={styles.summaryDetailText}>Serves {servings || '?'}</Text>
            <Text style={styles.summaryDetailText}>
              {servingSizeAmount || '?'} {servingSizeUnit ?? '?'} / serving
          ...
          {/* Real steps, edit mode -- 2026-08-17. Its own bordered card,
              ...
              saved handheld to fix something is exactly as much "the area
  ...
  // Continue, and again right after each "Add to Handheld") -- no separate
  ...
  if (servingsConfirmed && finishStep === 'building' && !pendingResolved) {
      ...
      return (
        <View style={styles.pickerScreen}>
          ...
          <FoodLookup
            ...
            allowedCategories={HANDHELDS_BUILDER_CATEGORIES}
    ...
    // Nothing chosen yet -- the collapsed handheld/ingredients card (point
    // 2) plus a real, NAVIGATION_HAND-aware "Add Ingredients" button
    // (points 3/5) opening the 3-way source chooser.
    return (
      <View style={styles.pickerScreen}>
        <CollapsibleOverlayCard
          collapsedLabel={handheldName.trim() || 'Handheld'}
          tabColor={tabColor}
    ...
    <>
      ...
      {!servingsConfirmed ? (
        <View style={[styles.formCard, { borderColor: tabColor }]}>
          ...
          {!editHandheldId && !fromFavoriteId ? (
            <View style={styles.findRecipeSection}>
              ...
              <TouchableOpacity style={styles.findRecipeLink} onPress={() => router.push({ pathname: '/purple-digest', params: { openDigestLens: 'myKitchen' } })}>
                <Text style={styles.findRecipeLinkText} numberOfLines={1}>My Kitchen (your saved handhelds)</Text>
          ...
          {/* Handheld Name, 2026-07-28 -- its own full-width field above
              Servings/Size, since it's not part of either's own row/
              column pairing and applies to the handheld as a whole. */}
          <Text style={[styles.formLabel, { color: tabColor }]}>Handheld Name</Text>
          ...
          <View style={[styles.dishNameFieldWrap, { backgroundColor: inputBackground(tabColor) }]}>
            {NAVIGATION_HAND === 'left' ? (
              <>
                <VoiceInputButton onResult={(transcript) => handleHandheldNameChange(transcript)} size={22} />
                <AppTextInput
                  style={[styles.formInput, styles.dishNameInputEmbedded]}
                  value={handheldName}
                  onChangeText={handleHandheldNameChange}
                  placeholder="e.g., Marinara Handheld"
          ...
          <TouchableOpacity
            style={[
              ...
              handheldFormReady ? { backgroundColor: colors.buttonColor } : styles.primaryButtonMuted,
            ]}
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {/* Header, 2026-07-31 -- "Preparation Steps for X", title-cased.
                  ...
                  correct them). titleCaseHandheldName is the same helper the
                  Handheld Name field uses, so the two can't drift apart. */}
              <View style={styles.pendingFoodRow}>
                ...
                <Text style={styles.pendingHeader} numberOfLines={2}>
                  Preparation Steps for {titleCaseHandheldName(foodSummary(pendingResolved))}
                </Text>
              ...
              {/* Four stacked labeled fields, 2026-07-31 -- Quantity,
                  ...
                  own nutrition, so a handheld built without them can't be
                  scored honestly. */}
              <View style={styles.labeledPickerRow}>
              ...
              {/* Real, per-ingredient Prep Steps, 2026-08-17 (point 6:
                  ...
                  Handheld"/"Save Ingredient" time (see saveIngredient's own
              ...
              {/* Edit mode: one "Save Ingredient" button -- 'add-new' vs.
                  ...
                  saveIngredient's own editHandheldId branch: both just commit
                  this ingredient and return to the overview screen), so
                  showing a two-button split that used to mean "add
                  another" vs. "finish the whole handheld" would just be
                  confusing now that finishing/persisting happens from the
                  overview instead. Create mode keeps the original Save &
                  Add New (left) / Save & Finish Handheld (right) pair,
                  2026-07-31 -- replaces the old Change Food + Add to Handheld
            ...
            // nudgeDismissed's own comment above): plenty of real handhelds
            ...
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={styles.emptyText}>
                No {missingExtras.map((extra) => extra.label).join(' or ')} logged for this handheld yet.
              </Text>
              <View style={styles.buttonRow}>
                <TouchableOpacity
                  style={styles.secondaryButton}
            ...
            // created for that handheld, allowing them to see how it looks,
            ...
            // the extras nudge already satisfied/dismissed. finishHandheld
            ...
            // 'building' for editHandheldId), which is why editHandheldId ?
            // 'Save Changes' below is a defensive fallback, not something
            // this branch is actually expected to render.
            <>
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={[styles.overviewHandheldName, { color: tabColor }]}>{handheldName.trim() || 'Handheld'}</Text>
              <Text style={styles.summaryDetailText}>
              ...
              {/* Real, live-computed nutrition/condition preview, point 7 --
                  the exact same real detail a saved handheld's own Digest My
                  Kitchen card shows, computed here BEFORE this handheld is
                  ever saved (see nutritionHighlights/conditionNotes' own
                  effect above). Both silently render nothing when empty
                  rather than an empty box -- a handheld with no real
            ...
            {nutritionHighlights.length > 0 ? (
              <View style={styles.bandOut}>
                <HomeSectionBand
                  title="What This Handheld Gives You"
            ...
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              <TouchableOpacity
                ...
                disabled={computingReport}
              >
                <Text style={styles.primaryButtonText}>{editHandheldId ? 'Save Changes' : 'Complete & Save This Handheld'}</Text>
              </TouchableOpacity>
            </View>
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  // Extra room above the Handheld Name form's own Continue button, 2026-07-28
  ...
  // collapsible handheld/ingredients card above it, and pinned to
  ...
  // this one is a real, different, horizontal divider for the new
