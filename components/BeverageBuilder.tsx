...
//
// 2026-08-02, explicitly reconsidered for this builder specifically
// (liters were reported missing, which led into this): a person's own
// profile system is the right rule for something like a body-weight scale,
// which really is set to one system at a time -- but it's the wrong rule
// for which measuring UTENSIL happens to be on hand for a drink. A liter
// jug, a gallon jug, a cup marked in mL vs. fl oz -- what's physically
// available doesn't track someone's app-wide preference the way a scale
// reading does. So for this builder, every liquid unit (both families) is
// offered together, unconditionally; only the MASS unit (grams vs.
// ounces/pounds -- genuinely tied to which scale is in use) still follows
// the profile system. lib/unitConversion.ts's own VolumeUnit/VOLUME_TO_ML
// already supports every unit below (pint/quart/gallon added alongside
// this change, US customary throughout, matching the existing fl_oz
// figure) -- this file only had to stop filtering, not add any new
// conversion math.
...
function formatFinalIngredientText(ingredient: BeverageIngredient): string {
...
export function BeverageBuilder({
  ...
  // Whether the collapsible beverage/ingredients card (point 2) is currently
  ...
  // whole add/edit/remove/complete flow) -- included in finishBeverage's own
  ...
  // the unchanged BeverageIngredient.prepNote/BeverageIngredientInput.prepNote
  ...
  // Curated starter recipes (2026-08-09, extended to Beverage Builder
  ...
  // Find a Recipe" further down in the render return. handlePickCuratedRecipe/
  ...
  useEffect(() => {
    if (openRecipeId && !editBeverageId && !fromFavoriteId) {
  ...
  // identical real detail before Save & Finish Beverage is ever tapped.
  ...
  function renderStepsSection(labelled = true) {
    return (
      <StepsEditor
        ...
        placeholder="e.g., Brew, steep, or blend, then chill or serve immediately."
  ...
  function renderLabeledPicker(
    ...
    // the beverage-naming screen's own three fields (#-of-Servings/Serving
  ...
  // The collapsible beverage/ingredients card's own content, 2026-08-17
  ...
  // both are real "review the whole beverage so far" moments. Full-width,
    ...
    return (
      <>
        {/* 2026-08-17, direct report: "it already has the name of the side
            ...
            beverageName as collapsedLabel right above this section. Serves/
            serving-size and the "tap to change beverage details" link
        ...
        >
          ...
          <Text style={[styles.secondaryButtonText, { color: tabColor, marginTop: 4 }]}>Tap to change beverage details</Text>
        </TouchableOpacity>
        ...
        {/* Edit mode: this card only ever shows while addingIngredient is
            ...
            > 0 gate) -- a beverage being edited already has ingredients by
      ...
      <>
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          {/* Real steps, edit mode -- 2026-08-17. Its own bordered card,
              ...
              saved beverage to fix something is exactly as much "the area
  ...
  if (servingsConfirmed && finishStep === 'building' && !pendingResolved) {
    ...
    // SOURCE_CANCEL_ROW_HEIGHT's own comment). allowedCategories/
    // allowedSubcategories still resolve through BEVERAGE_SUBTYPE_CONFIG
    // whenever a real subtype was chosen -- unchanged from before this
    // redesign, just carried into the restricted picker instead of the old
    // unconditional one.
    ...
    // Nothing chosen yet -- the collapsed beverage/ingredients card (point 2)
    ...
    return (
      <View style={styles.pickerScreen}>
        <CollapsibleOverlayCard
          collapsedLabel={beverageName.trim() || 'Beverage'}
          tabColor={tabColor}
        ...
        <View style={[styles.formCard, { borderColor: tabColor }]}>
            ...
            <View style={styles.findRecipeSection}>
              ...
              >
                <Text style={styles.findRecipeLinkText} numberOfLines={1}>
                  My Recipes (your saved beverages)
                </Text>
          ...
          <TouchableOpacity
            style={[
              ...
              beverageFormReady ? { backgroundColor: colors.buttonColor } : styles.primaryButtonMuted,
            ]}
        ...
        <>
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {/* 2026-08-17 -- replaces the old single free-text Prep Notes
                  field with the same real StepsEditor used for the whole
                  beverage's own Steps, so someone can lay out real,
                  numbered prep steps for one ingredient ("dice small,"
                  "set aside") rather than one run-on sentence. Not
                  required: Cut Prep and Cook Prep still carry what
                  actually changes nutrition; this is extra recipe-style
                  detail on top. */}
              <View style={{ marginTop: 7 }}>
                <StepsEditor
                  ...
                  addAnotherLabel="+ Add Another Step"
                  completeLabel="Steps Complete"
                  placeholder="e.g., Steep for 5 minutes, then strain."
                  scrollViewRef={scrollViewRef}
                />
              </View>
            ...
            // created for that beverage, allowing them to see how it looks,
            ...
            // the extras nudge already satisfied/dismissed. finishBeverage is
            ...
            // 'building' for editBeverageId), which is why editBeverageId ?
            // 'Save Changes' below is a defensive fallback, not something
            // this branch is actually expected to render.
            <>
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={[styles.overviewBeverageName, { color: tabColor }]}>{beverageName.trim() || 'Beverage'}</Text>
              <Text style={styles.summaryDetailText}>
              ...
              {/* Real, live-computed nutrition/condition preview, point 7 --
                  the exact same real detail a saved beverage's own Digest My
                  Kitchen card shows, computed here BEFORE this beverage is
                  ever saved (see nutritionHighlights/conditionNotes' own
                  effect above). Both silently render nothing when empty
                  rather than an empty box -- a beverage with no real
            ...
            {nutritionHighlights.length > 0 ? (
              <View style={styles.bandOut}>
                <HomeSectionBand
                  title="What This Beverage Gives You"
...
const styles = StyleSheet.create({
  ...
  // collapsible beverage/ingredients card above it, and pinned to whichever
