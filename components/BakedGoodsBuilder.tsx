...
export function BakedGoodsBuilder({
  ...
  useEffect(() => {
    let isCurrent = true;
    listCuratedRecipes('bakedGoods').then((recipes) => {
  ...
  async function finishBakedGoods(finalIngredients: BakedGoodsIngredient[]) {
    ...
    <>
      ...
      {!servingsConfirmed ? (
        <View style={[styles.formCard, { borderColor: tabColor }]}>
          ...
          {curatedRecipes.length > 0 && !editBakedGoodsId && !fromFavoriteId ? (
