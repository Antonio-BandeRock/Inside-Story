...
//
// 2026-08-02, explicitly reconsidered for this builder (broths/stocks are
...
// happens to be on hand for a pot of soup. A liter jug, a gallon jug, a cup
...
// The finished soup's own ingredient list, 2026-08-17 -- see
// SideBuilder.tsx's own identical helper for the full reasoning: reused by
// the final recipe-card review screen below, formatting each real
// ingredient as "quantity unit food (cut, cook)" -- N/A cut/cook values are
// dropped rather than shown as a real answer, since "N/A" describes the
// absence of a meaningful prep state, not a genuine detail worth repeating.
function formatFinalIngredientText(ingredient: SoupIngredient): string {
...
export function SoupBuilder({
  ...
  // 2026-08-08. Same shape as editSoupId's own prefill just below, except
  // this never marks anything as an edit -- finishSoup always creates a
  // genuinely NEW soup from a favorite.
  fromFavoriteId,
  ...
  // whole add/edit/remove/complete flow) -- included in finishSoup's own
  ...
  // the unchanged SoupIngredient.prepNote/SoupIngredientInput.prepNote
  ...
  // Same shape as editSoupId's own effect just above -- see
  // SideBuilder.tsx's own identical fromFavoriteId effect for the full
  // reasoning. Never sets editSoupId-equivalent state, so finishSoup below
  // always creates a new soup.
  useEffect(() => {
  ...
  // Curated starter recipes (2026-08-09, extended to Soup Builder
  ...
  useEffect(() => {
    if (openRecipeId && !editSoupId && !fromFavoriteId) {
  ...
  // identical real detail before Save & Finish Soup is ever tapped.
  ...
  // either way.
  //
  // Shared by finishSoup and handlePreviewReport below -- see
  // SideBuilder.tsx's own identical helper for the full reasoning.
  function buildDepthIngredients(finalIngredients: SoupIngredient[]): MealIngredientInput[] {
  ...
  async function finishSoup(finalIngredients: SoupIngredient[], precomputedDepth?: RecipeDepthResult) {
    ...
    const payload = {
      ...
      // what was explicitly saved counts" rule Save & Finish Soup
    ...
    if (alsoSaveAsFavorite) {
      try {
        await saveBuilderFavorite('soup', payload);
      } catch (error) {
        console.error('[SoupBuilder] Failed to save favorite', error);
        showInfoAlert('Soup saved, favorite failed', `${finishedName} is saved, but saving it as a favorite didn't work. You can try favoriting it again later.`);
      }
    }
  ...
  async function handlePreviewReport() {
    ...
    try {
      ...
      console.error('[SoupBuilder] Failed to compute the depth report', error);
      showInfoAlert('Report failed', 'Something went wrong building the report. You can still save this soup directly.');
    } finally {
  ...
  function renderStepsSection() {
    return (
      <StepsEditor
        ...
        placeholder="e.g., Sauté the onions and garlic first, then add the broth and simmer for 20 minutes."
  ...
  function renderLabeledPicker(
    ...
    // the soup-naming screen's own three fields (#-of-Servings/Serving
  ...
  // The collapsible soup/ingredients card's own content, 2026-08-17 (point
  ...
  // real "review the whole soup so far" moments. Full-width, single
    ...
    return (
      <>
        {/* 2026-08-17, direct report: "it already has the name of the side
            ...
            soupName as collapsedLabel right above this section. Serves/
            serving-size and the "tap to change soup details" link stay --
        ...
        >
          ...
          <Text style={[styles.secondaryButtonText, { color: tabColor, marginTop: 4 }]}>Tap to change soup details</Text>
        </TouchableOpacity>
        ...
        {/* Edit mode: this card only ever shows while addingIngredient is
            ...
            > 0 gate) -- a soup being edited already has ingredients by
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          <RecipeDepthReport
            ...
            onSave={() => {
              ...
              void finishSoup(ingredients, reportData).finally(() => setSavingFromReport(false));
            }}
          />
        </ScrollView>
        <AppActionSheet
            ...
            ...(stagePickerFor ? getConditionStagingModel(stagePickerFor.code)?.stages ?? [] : []).map((stage) => ({
              ...
              onPress: () => {
                ...
                setConditionStage(code, stage.code)
                  ...
                  .catch((error) => {
                    console.error('[SoupBuilder] Failed to save the declared healing stage', error);
      ...
      <>
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          {/* Real steps, edit mode -- 2026-08-17. Its own bordered card,
              ...
              saved soup to fix something is exactly as much "the area that
  ...
  if (servingsConfirmed && finishStep === 'building' && !pendingResolved) {
    ...
    // Nothing chosen yet -- the collapsed soup/ingredients card (point 2)
    ...
    return (
      <View style={styles.pickerScreen}>
        <CollapsibleOverlayCard
          collapsedLabel={soupName.trim() || 'Soup'}
          tabColor={tabColor}
        ...
        <View style={[styles.formCard, { borderColor: tabColor }]}>
          ...
          <View style={[styles.dishNameFieldWrap, { backgroundColor: inputBackground(tabColor) }]}>
            {NAVIGATION_HAND === 'left' ? (
              <>
                <VoiceInputButton onResult={(transcript) => handleSoupNameChange(transcript)} size={22} />
                <AppTextInput
            ...
            <View style={styles.findRecipeSection}>
              ...
              >
                <Text style={styles.findRecipeLinkText} numberOfLines={1}>
                  My Kitchen (your own saved soups)
                </Text>
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {/* Real, per-ingredient Prep Steps, 2026-08-17 (point 6:
                  ...
                  Soup"/"Save Ingredient" time (see saveIngredient's own
              ...
              <View style={{ marginTop: 7 }}>
                <StepsEditor
                  ...
                  addAnotherLabel="+ Add Another Step"
                  completeLabel="Steps Complete"
                  placeholder="e.g., Peel and chop finely, add on top after simmering"
                  scrollViewRef={scrollViewRef}
                />
              </View>
            ...
            // created for that soup, allowing them to see how it looks,
            ...
            // the extras nudge already satisfied/dismissed. finishSoup is
            ...
            // 'building' for editSoupId), which is why editSoupId ?
            // 'Save Changes' below is a defensive fallback, not something
            // this branch is actually expected to render.
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={[styles.overviewSoupName, { color: tabColor }]}>{soupName.trim() || 'Soup'}</Text>
              <Text style={styles.summaryDetailText}>
              ...
              {/* Real, live-computed nutrition/condition preview, point 7 --
                  the exact same real detail a saved soup's own Digest My
                  Kitchen card shows, computed here BEFORE this soup is ever
                  saved (see nutritionHighlights/conditionNotes' own effect
                  above). Both silently render nothing when empty rather
                  than an empty box -- a soup with no real standout nutrient
              ...
              {nutritionHighlights.length > 0 ? (
                <View style={[styles.recipeNutritionBox, { borderColor: tabColor }]}>
                  <Text style={[styles.recipeNutritionLabel, { color: tabColor }]}>What This Soup Gives You</Text>
...
const styles = StyleSheet.create({
  ...
  // 2026-08-17 -- see the Soup Name field's own comment for why this
  ...
  // summaryRightColumn/summarySoupName lived here until 2026-08-17 -- the
  // old two-column summary card (soup info on the left, a scrollable
  ...
  // collapsible soup/ingredients card above it, and pinned to whichever
  ...
  // separating the soup-info block from the Ingredients list beneath it --
  ...
  // "What This Soup Gives You" -- matches RecipeCardDetail's own real
