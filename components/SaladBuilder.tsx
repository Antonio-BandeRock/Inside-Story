...
// titleCaseSaladName below), matching standard title-case convention.
const SALAD_NAME_MINOR_WORDS = new Set([
...
function titleCaseSaladName(text: string): string {
  ...
  return text.replace(/[A-Za-z']+/g, (word) => {
    ...
    if (!first && SALAD_NAME_MINOR_WORDS.has(lower)) return lower;
...
// Reverses parseAmountValue for a saved salad being reopened for editing --
...
// in the first place (shouldn't happen for anything SaladBuilder itself
...
// Asked per INGREDIENT, not once for the whole salad, 2026-07-29 -- a real
// salad routinely combines ingredients that were each prepared
// completely differently (garlic added raw as a topping after the rest of
// the salad is cooked; bacon pan-fried into bits in its own separate step
...
// salad-wide answer was actively wrong for any salad like that, not just
// imprecise. Required, not optional the way Salad Name is, for the same
// nutrition reason.
//
...
type SaladIngredient = {
  ...
  quantity: string;
  unit: string;
  // How THIS ingredient will be prepared for THIS salad -- see
  // COOKING_METHODS' own comment for why this moved from a single
  // salad-wide question to a per-ingredient one. cookingMethod is required
...
function formatFinalIngredientText(ingredient: SaladIngredient): string {
...
// Builds a single salad from one or more ingredients -- Category ->
...
//
// The in-progress salad (saladName/servings/ingredients/etc.) is local
// component state only, same as before -- a salad isn't a real, saved
// record until Save & Finish Salad actually commits it (see finishSalad/
// saveSalad in lib/db.ts, 2026-08-01). Navigating away from a salad mid-
// build still loses it; that's still a known, accepted gap, just a
// narrower one now that finishing a salad for real is possible.
//
...
// ("Salad Builder") was removed -- PageIdentityLabel's own bottom-
...
// place, once Salad Name/Servings/Size is confirmed, that form splits into
// a compact two-column summary card (salad info on the left, a scrollable
...
// first fully-built builder, and the proven, on-device-tested pattern all
// nine remaining builders are meant to follow -- see CLAUDE.md's Food tab
// section) rather than designing this screen's own layout from scratch. The
// comments below that carry earlier 2026-07-xx dates describe reasoning
// SideBuilder itself arrived at through real iteration (dropped title bar,
// PopoverSelect replacing the old wheel picker, NAVIGATION_HAND-aware field
// reordering, etc.) -- inherited here as still-accurate design reasoning,
// not a claim that SaladBuilder went through that same history itself.
// The one genuinely new, salad-specific piece is the raw-goitrogenic-load
// warning at finish time (see findRawGoitrogenicIngredients/
// confirmAndFinishSalad below) -- mixing several raw goitrogenic vegetables
// into one salad makes it easy to eat far more of them at once than anyone
// would eat cooked and separate, the one concrete risk this builder's own
// placeholder copy in app/(tabs)/food.tsx was written to call out.
export function SaladBuilder({
  tabColor,
  // Set when reached via the Edit button on an already-saved salad (see
  // app/food-items.tsx) -- 2026-08-01. Loads that salad's real data into
  ...
  // machinery a fresh salad already uses. finishSalad below branches on this
  // to call updateSalad (in place) instead of saveSalad (a new row), and to
  // navigate back to the list afterward instead of resetting to blank --
  // "I fixed this salad" should return you to where you came from, not
  // drop you into building a different one.
  editSaladId,
  // Set when reached via "Use this Favorite" on a saved favorite (see
  // app/food-items.tsx) -- 2026-08-08. Same shape as editSaladId's own
  // prefill just below, except this never marks anything as an edit --
  // Save & Finish Salad always creates a genuinely NEW salad from a
  ...
  // field itself, or Change Food/Add to Salad below it) could end up stuck
  ...
  // falls back to a plain "Salad" in the summary/ingredient views
  // (saladName.trim() || 'Salad') as a harmless extra safety net, even
  ...
  // here (`(text) => setSaladName(titleCaseSaladName(text))`, passed directly
  ...
  // renders (titleCaseSaladName is a plain module-level function, setSaladName
  // is already a stable state setter), breaking the loop.
  const handleSaladNameChange = useCallback((text: string) => {
    setSaladName(titleCaseSaladName(text));
  }, []);
  ...
  // made). Explicitly corrected: every field in this whole form -- Salad
  ...
  // below) -- true only once Salad Name and all three pickers are actually
  // filled in/chosen.
  const saladFormReady = !!saladName.trim() && !!servings && !!servingSizeAmount && !!servingSizeUnit;
  ...
  // Whether the collapsible salad/ingredients card (point 2) is currently
  ...
  // whole add/edit/remove/complete flow) -- included in finishSalad's own
  ...
  // Carried onto the SaladIngredient at add time rather than re-fetched
  ...
  // servingSizeUnit above. "Add to Salad" below stays disabled until
  ...
  // SaladIngredient's own comment for why this replaced a single
  // salad-wide question. Reset after each "Add to Salad," same as
  ...
  // the unchanged SaladIngredient.prepNote/SaladIngredientInput.prepNote
  ...
  // NEXT ingredient after "Add to Salad," both land back on that same
  // category instead of a blank Category list -- a salad very often
  ...
  // Loads an existing salad's real data in place of the blank-builder
  // defaults above, 2026-08-01 -- runs once per editSaladId. salad_ingredients
  ...
  useEffect(() => {
    if (!editSaladId) return;
    let isCurrent = true;

    (async () => {
      const salad = await getSalad(editSaladId);
      if (!salad || !isCurrent) return;

      const details = await getSaladIngredients(editSaladId);
      ...
      setServings(formatAmountForPicker(salad.servings, SERVINGS_PICKER_VALUES));
      setServingSizeAmount(formatAmountForPicker(salad.servingSizeAmount, AMOUNT_PICKER_VALUES));
      setServingSizeUnit(salad.servingSizeUnit);
  ...
  // Same shape as editSaladId's own effect just above -- reconstructs each
  ...
  // SaladIngredientInput's own field set). Never sets editSaladId-
  // equivalent state, so finishSalad below always creates a new salad.
  useEffect(() => {
  ...
  // below) -- that Quantity/Add-to-Salad card's own Quantity field
  ...
  // the same instant. Without scrolling, "Add to Salad" -- the very last
  ...
  // Salad does nothing," which is exactly what a covered button looks like.
  ...
  // Starts false so opening an existing salad for editing always lands on
  // the ingredient overview first, not a "pick a Category" prompt --
  // explicitly requested, since re-opening a salad is almost always to
  ...
  // flagged, which is wrong for genuinely oil-free/seasoning-free salads
  ...
  // identical real detail before Save & Finish Salad is ever tapped.
  ...
  // order they appear on screen (Salad Name, then Servings, then Serving
  ...
  // 2026-07-28, including the exact wording for Salad Name/Servings; the
  // other two follow the same "Please choose/enter..." phrasing.
  function handleContinuePress() {
    if (!saladName.trim()) {
      showInfoAlert('Almost there', 'Please enter a name for this salad.');
      return;
    }
    if (!servings) {
      showInfoAlert('Almost there', 'Please choose the # of Servings this salad will contain.');
      return;
    }
    if (!servingSizeAmount) {
      showInfoAlert('Almost there', 'Please choose the Serving Size for this salad.');
      return;
    }
    if (!servingSizeUnit) {
      showInfoAlert('Almost there', 'Please choose the Units for this salad.');
      return;
    }
    dismissKeyboard();
    setServingsConfirmed(true);
  ...
  // catch (see FOOD_LENS_COPY.saladBuilder in app/(tabs)/food.tsx, and this
  // file's own top comment): mixing several raw goitrogenic/cruciferous
  // vegetables into one salad makes it easy to eat a much larger raw
  ...
  // already fetched and carried on it at add time (see SaladIngredient's
  // own `scores` comment), rather than re-querying anything.
  function findRawGoitrogenicIngredients(ingredientList: SaladIngredient[]): string[] {
  ...
  // Gates finishSalad behind the raw-goitrogenic check above, 2026-08-02 --
  // every "finish" button in this builder (create, edit, and the ready
  // screen) goes through this instead of calling finishSalad directly. Never
  // blocks saving outright (plenty of real salads genuinely combine two raw
  ...
  async function confirmAndFinishSalad(finalIngredients: SaladIngredient[]) {
    ...
    if (flaggedFoods.length >= 2) {
      const ok = await confirmSheet({
        ...
        message: `This salad combines ${flaggedFoods.length} raw goitrogenic vegetables (${flaggedFoods.join(', ')}). Mixing them raw makes it easy to eat far more of these at once than you'd eat cooked and separate. Consider cooking one first, or using less of them.`,
  ...
  // THEN resets the builder back to a blank salad, so a save failure never
  ...
  async function finishSalad(finalIngredients: SaladIngredient[]) {
    ...
    const ingredientInputs: SaladIngredientInput[] = finalIngredients.map((ingredient) => ({
    ...
    try {
      if (editSaladId) {
        await updateSalad(editSaladId, payload);
      } else {
        await saveSalad(payload);
      }
    } catch (error) {
      console.error('[SaladBuilder] Failed to save salad', error);
      showInfoAlert('Save failed', 'Something went wrong saving this salad. Your ingredients are still here; please try again.');
      return;
    }
    ...
    if (alsoSaveAsFavorite) {
      try {
        await saveBuilderFavorite('salad', payload);
      } catch (error) {
        console.error('[SaladBuilder] Failed to save favorite', error);
        showInfoAlert('Salad saved, favorite failed', `${finishedName} is saved, but saving it as a favorite didn't work. You can try favoriting it again later.`);
      }
    }

    // Editing an already-saved salad returns to wherever it was opened from
    // (see app/food-items.tsx's Edit button, which pushed this screen) --
    // "I fixed this salad" should go back to the list, not drop the person
    ...
    // Back to a blank salad. From here the Lens Button starts a
  ...
  // or ends the salad entirely -- 2026-07-31, the two Save buttons.
  //
  //   'add-new' -> back to picking a Food Category, then a food, and round
  //                again, indefinitely, until the person chooses 'finish'.
  //   'finish'  -> saves the whole salad (see finishSalad above) and resets
  //                the builder to a blank new salad.
  function saveIngredient(then: 'add-new' | 'finish') {
    if (!pendingResolved || !quantity || !unit || !ingredientCutPrep || !ingredientCookingMethod) {
      ...
      // appear on screen -- same approach as the salad form's own Continue.
      const missing = [
        !quantity && 'Quantity',
        !unit && 'Units',
    ...
    dismissKeyboard();
    const newIngredient: SaladIngredient = {
    ...
    if (then === 'finish') {
      confirmAndFinishSalad(allIngredients);
    }
  }
  ...
  // salad (the small X row-remove buttons, packed close together in this
  ...
  function renderStepsSection() {
    return (
      <StepsEditor
        ...
        placeholder="e.g., Toss the greens with the dressing just before serving."
  ...
  // The Salad/Ingredients summary card -- salad name/servings/size on the
  ...
  function renderLabeledPicker(
    ...
    // the salad-naming screen's own three fields (#-of-Servings/Serving
  ...
  // The collapsible salad/ingredients card's own content, 2026-08-17 (point
  ...
  // real "review the whole salad so far" moments. Full-width, single
    ...
    return (
      <>
        {/* 2026-08-17, direct report: "it already has the name of the side
            ...
            saladName as collapsedLabel right above this section. Serves/
            serving-size and the "tap to change salad details" link stay --
        ...
        >
          ...
          <Text style={[styles.secondaryButtonText, { color: tabColor, marginTop: 4 }]}>Tap to change salad details</Text>
        </TouchableOpacity>
        ...
        {/* Edit mode: this card only ever shows while addingIngredient is
            ...
            > 0 gate) -- a salad being edited already has ingredients by
  ...
  // (the salad-name row only shows while !servingsConfirmed; the ingredient
  ...
  const saladFormFields = useReorderedLabeledFields([
  ...
  // here by default (addingIngredient starts false) shows the salad info
  ...
  // ahead of only when editSaladId is set).
  if (editSaladId && servingsConfirmed && !pendingResolved && !addingIngredient) {
    return (
      <>
        {infoAlertElement}
        {confirmSheetElement}
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={[styles.scrollContent, { paddingBottom: scrollBottomPadding }]}>
          ...
          >
            <Text style={[styles.overviewSaladName, { color: tabColor }]} numberOfLines={2}>
              {saladName.trim() || 'Salad'}
            </Text>
            <Text style={styles.summaryDetailText}>Serves {servings || '?'}</Text>
            <Text style={styles.summaryDetailText}>
              {servingSizeAmount || '?'} {servingSizeUnit ?? '?'} / serving
          ...
          {/* Real steps, edit mode -- 2026-08-17. Its own bordered card,
              ...
              saved salad to fix something is exactly as much "the area that
          ...
          <TouchableOpacity style={[styles.primaryButton, { backgroundColor: tabColor }]} onPress={() => confirmAndFinishSalad(ingredients)}>
  ...
  // Continue, and again right after each "Add to Salad") -- no separate
  ...
  if (servingsConfirmed && finishStep === 'building' && !pendingResolved) {
    ...
    // Nothing chosen yet -- the collapsed salad/ingredients card (point 2)
    ...
    return (
      <View style={styles.pickerScreen}>
        <CollapsibleOverlayCard
          collapsedLabel={saladName.trim() || 'Salad'}
          tabColor={tabColor}
    ...
    <>
      ...
      {!servingsConfirmed ? (
        <View style={[styles.formCard, { borderColor: tabColor }]}>
          ...
          <Text style={[styles.formLabel, { color: tabColor }]}>Salad Name</Text>
          ...
          <View style={[styles.dishNameFieldWrap, { backgroundColor: inputBackground(tabColor) }]}>
            {NAVIGATION_HAND === 'left' ? (
              <>
                <VoiceInputButton onResult={(transcript) => handleSaladNameChange(transcript)} size={22} />
                <AppTextInput
                  style={[styles.formInput, styles.dishNameInputEmbedded]}
                  value={saladName}
                  onChangeText={handleSaladNameChange}
                  placeholder="e.g., Kale & Chickpea Salad"
          ...
          <TouchableOpacity
            style={[
              ...
              saladFormReady ? { backgroundColor: tabColor } : styles.primaryButtonMuted,
            ]}
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {/* Header, 2026-07-31 -- "Preparation Steps for X", title-cased.
                  ...
                  correct them). titleCaseSaladName is the same helper the
                  Salad Name field uses, so the two can't drift apart. */}
              <View style={styles.pendingFoodRow}>
                ...
                <Text style={styles.pendingHeader} numberOfLines={2}>
                  Preparation Steps for {titleCaseSaladName(foodSummary(pendingResolved))}
                </Text>
              ...
              {/* Four stacked labeled fields, 2026-07-31 -- Quantity,
                  ...
                  own nutrition, so a salad built without them can't be
                  scored honestly. */}
              <View style={styles.labeledPickerRow}>
              ...
              {/* Real, per-ingredient Prep Steps, 2026-08-17 (point 6:
                  ...
                  Salad"/"Save Ingredient" time (see saveIngredient's own
              ...
              <View style={{ marginTop: 7 }}>
                <StepsEditor
                  ...
                  addAnotherLabel="+ Add Another Step"
                  completeLabel="Steps Complete"
                  placeholder="e.g., Peel and chop finely, add on top after tossing"
                  scrollViewRef={scrollViewRef}
                />
              </View>
            ...
            // nudgeDismissed's own comment above): plenty of real salads
            ...
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={styles.emptyText}>
                No {missingExtras.map((extra) => extra.label).join(' or ')} logged for this salad yet.
              </Text>
              <View style={styles.buttonRow}>
                <TouchableOpacity
                  style={styles.secondaryButton}
            ...
            // created for that salad, allowing them to see how it looks,
            ...
            // the extras nudge already satisfied/dismissed. confirmAndFinishSalad
            ...
            // 'building' for editSaladId), which is why editSaladId ?
            // 'Save Changes' below is a defensive fallback, not something
            // this branch is actually expected to render.
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={[styles.overviewSaladName, { color: tabColor }]}>{saladName.trim() || 'Salad'}</Text>
              <Text style={styles.summaryDetailText}>
              ...
              {/* Real, live-computed nutrition/condition preview, point 7 --
                  the exact same real detail a saved salad's own Digest My
                  Kitchen card shows, computed here BEFORE this salad is ever
                  saved (see nutritionHighlights/conditionNotes' own effect
                  above). Both silently render nothing when empty rather
                  than an empty box -- a salad with no real standout nutrient
              ...
              {nutritionHighlights.length > 0 ? (
                <View style={[styles.recipeNutritionBox, { borderColor: tabColor }]}>
                  <Text style={[styles.recipeNutritionLabel, { color: tabColor }]}>What This Salad Gives You</Text>
...
const styles = StyleSheet.create({
  ...
  // 2026-08-17 -- see the Salad Name field's own comment for why this
  ...
  // Extra room above the Salad Name form's own Continue button, 2026-07-28
  ...
  // summaryRightColumn/summarySaladName lived here until 2026-08-17 -- the
  // old two-column summary card (salad info on the left, a scrollable
  // ingredient list on the right) that this whole redesign replaced with
  // CollapsibleOverlayCard (see renderSummaryCardContent's own comment,
  // this file's top). summaryDetailText is kept -- it's still used inside
  // that new component's own content.
  summaryDetailText: {
    ...typography.caption,
  ...
  // of summarySaladName/summaryIngredientRow/summaryRemoveButton above,
  ...
  // collapsible salad/ingredients card above it, and pinned to whichever
  ...
  // separating the salad-info block from the Ingredients list beneath it --
  ...
  // "What This Salad Gives You" -- matches RecipeCardDetail's own real
