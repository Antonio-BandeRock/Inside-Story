...
// The Salad/Ingredients summary card's own fixed footprint (see its own
...
export function SaladBuilder({
  ...
  // padding, and the divider plus its own two salad gaps -- split evenly,
  ...
  function confirmAndFinishSalad(finalIngredients: SaladIngredient[]) {
    const flaggedFoods = findRawGoitrogenicIngredients(finalIngredients);
    if (flaggedFoods.length >= 2) {
      Alert.alert(
        'Several raw goitrogenic vegetables together',
        `This salad combines ${flaggedFoods.length} raw goitrogenic vegetables (${flaggedFoods.join(', ')}). Mixing them raw makes it easy to eat far more of these at once than you'd eat cooked and separate. Consider cooking one first, or using less of them.`,
        [
          { text: 'Go back and adjust', style: 'cancel' },
          { text: 'Continue anyway', onPress: () => void finishSalad(finalIngredients) },
        ],
      );
      return;
    }
  ...
  function renderSummaryCard(squareBottom: boolean) {
    return (
      ...
      >
        ...
        >
          <Text style={[styles.summarySaladName, { color: tabColor }]} numberOfLines={2}>
        ...
        <View style={styles.summaryRightColumn}>
          ...
          <View style={styles.summaryDoneRow}>
            {/* Edit mode: this card only ever shows while addingIngredient
                ...
                ingredients.length > 0 gate) -- a salad being edited already
        ...
        <>
          ...
          {pendingResolved ? (
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              ...
              {editSaladId ? null : renderFavoriteToggle()}
            ...
            // which doesn't exist in this branch). finishSalad is called
            ...
            <View style={[styles.formCard, { borderColor: tabColor }]}>
              <Text style={styles.emptyText}>
                {(saladName.trim() || 'Salad')} ready with {ingredients.length} ingredient
              ...
              >
                <Text style={styles.primaryButtonText}>{editSaladId ? 'Save Changes' : 'Save & Finish Salad'}</Text>
              </TouchableOpacity>
            </View>
          )}
        </>
...
const styles = StyleSheet.create({
  ...
  // Two columns (salad info, scrollable ingredients) divided by a thin
  ...
  // 2026-07-28, explicitly requested -- salad name pinned to the top of
  ...
  // site (was 1), explicitly requested so a longer salad name wraps to a
