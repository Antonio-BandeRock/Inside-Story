...
**Most recent work (2026-08-20):** Wave 1 of a fermented-drinks workstream, direct request built from two shared Google AI Mode conversations about homemade fermented drinks for gut health, joint pain, and sleep. Every claim was independently fact-checked via WebSearch before being written in, not carried over from the source conversations as-is: several got downgraded (casein-thyroid "molecular mimicry" is a practitioner hypothesis, not consensus; resveratrol-Akkermansia is mostly animal-study evidence so far) and one citation caught mid-build was outright fabricated (a guessed PMC id for hibiscus/blood pressure resolved to an unrelated paper), replaced with a verified real one after independently re-checking every citation added this session. 22 new curated Fermentation Builder recipes shipped to the live `assets/data/foods_reference.db` (`scripts/add_fermented_drink_recipes.py`, every ingredient re-verified resolvable and not audit-hidden after running): a 10-variant Wild-Fermented Fruit/Flower Tonic family (tart cherry+ginger+turmeric is the flagship, built for sleep/joints/gut; blueberry, pomegranate, cranberry, red grape, hibiscus, blackberry-raspberry, elderberry, apple-pear, lemon-lime round it out) plus 12 named traditional drinks (Beet Kvass, Kanji, Water Kefir, Coconut Kefir, Ginger Bug Soda, Traditional Ginger Beer, Fermented Turmeric Drink, Tepache, Shrub, Switchel, Jun Tea, Fermented Garlic Honey Tonic), each with a full Digest `RecipeCard` (`lib/digest/recipes.ts`) and a "Build This Recipe" link into Fermentation Builder. Two new Basic Health entries (`lib/digest/fermentationMethods.ts`) cover the wild-ferment tepache-style method itself and a dairy-free/gluten-free survey of the ~19 named drinks not built as recipes this wave (Milk Kefir, Amazake, Rejuvelac, Mauby, Calpis, Burdock & Dandelion Ale, Pozol, Sobia, Pine Needle Cheong, Boza, Chicha, rye Kvass, Sake, Makgeolli, Ayran, Lassi, Tarag, Pu-erh Tea, Palm Wine, Pulque — a tracked Wave 2 backlog, several needing a real gluten-free grain substitution decision). A new Fermentation Tracker + "My Fermented Drinks" harvest-inventory data layer shipped in `lib/db.ts` (`fermentation_batches`/`fermentation_task_links`/`fermentation_harvests` tables, `startFermentationBatch`/`advanceFermentationBatch`/`recordFermentationHarvest`/`recordFermentationHarvestUsage`/`markFermentationHarvestFinished` and friends), mirroring Garden's own harvest/task-link precedent; real stir/burp/taste-test reminder series generate through the existing `insertScheduleSeries` machinery. `npx tsc --noEmit` and `npx eslint` both clean on every touched file. **Not yet built:** the Tracker's own UI screen (planned to hang off Fermentation Builder's save/detail screen), the 19-condition "which fermented drinks help this condition" crosslink entries, and on-device confirmation of any of this — all queued, not forgotten (see Open Next Steps below).
...
12. **Fermented Drinks workstream, Waves 2+ and the Tracker UI**, started 2026-08-20 (see Status snapshot above for what Wave 1 shipped). Standing backlog: ~19 more named traditional drinks (Milk Kefir, Amazake, Rejuvelac, Mauby, Calpis, Burdock & Dandelion Ale, Pozol, Sobia, Pine Needle Cheong, Boza, Chicha, rye Kvass, Sake, Makgeolli, Ayran, Lassi, Tarag, Pu-erh Tea, Palm Wine, Pulque — several need a real gluten-free grain substitution decision, a few may have no resolvable reference-database ingredient at all); the Fermentation Tracker's own UI screen (data layer is done, "Track this batch" screen off Fermentation Builder's save/detail view is not); and one new "Fermented Drinks & Foods for [Condition]" Digest entry per tracked condition (all 19), cross-linked both directions with the new Basic Health/Recipes content.
