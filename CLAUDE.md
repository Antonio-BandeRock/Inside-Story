...
**Fermentation Builder now asks "what kind of fermentation" first**, direct correction: "the answer to that question provides what is needed to filter what the next screen will provide. The bacterias and probiotics selections do not belong here at all. Those are ingredients for a yogurt fermentation, and that is how they should be treated." Built as a direct mirror of Beverage Builder's own already-shipped subtype-picker pattern (`components/FermentationSubtypePicker.tsx`, five real types: Fruit & Vegetable Tonics, Kombucha/Tea/Grain Ferments, Water & Coconut Kefir, Milk Kefir/Yogurt/Cultured Dairy, Something Else), each narrowing which ingredient categories `FoodLookup` offers. The Cultures & Probiotics strain picker now only shows for the Milk Kefir/Yogurt subtype, the one type it actually describes — every other subtype's own real culture (a SCOBY, kefir grains, wild yeast) was never one of this app's 7 catalogued strains anyway. Editing/reusing an existing fermentation or building from a curated recipe skips the picker outright, same bypass logic Beverage already established. `tsc`/`eslint` clean, confirmed live in a forced bundle. **Not yet confirmed on-device.**

**Not yet done overall:** on-device confirmation of the fermented-drinks recipes/Tracker/harvest flow, individual confirmation of bug-hunt fixes 1-5, and on-device confirmation of the new Fermentation subtype picker.
