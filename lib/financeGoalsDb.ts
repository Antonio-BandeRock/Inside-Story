...
export async function listGoalsWithProgress(): Promise<GoalWithProgress[]> {
  ...
  const totalByCost: Record<string, number> = {};
  for (const row of contribRows) {
    (contributionsByCost[row.costId] ??= []).push(row);
    totalByCost[row.costId] = (totalByCost[row.costId] ?? 0) + row.amount;
  }
  ...
  return goalRows.map((row) => {
    ...
    const costs = (costsByGoal[goal.id] ?? []).map((cost) => costProgress(cost, totalByCost[cost.id] ?? 0));
