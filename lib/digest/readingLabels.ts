...
export const READING_LABELS_ENTRIES: DigestEntry[] = [
  {
    ...
    teaser: 'Every number and every ingredient on a US food label follows a specific federal rule. Knowing the rule is what makes the label readable.',
    summary:
      'The Nutrition Facts panel and ingredient list on a packaged food sold in the US are both governed by specific FDA regulations, not a manufacturer\'s free choice of what to show or how to show it. That structure is exactly what makes a label decodable once the underlying rules are known: the ingredient list always follows one fixed ordering rule, the Daily Value percentages are always calculated the same way regardless of the product, and specific allergens must always be disclosed in one of two specific formats. These entries cover those mechanics directly, so a label reads as information rather than marketing.',
    citations: [
      {
  ...
  },
  {
    ...
    summary: 'Every number on a Nutrition Facts panel, calories, sugar, sodium, everything, is reported per the manufacturer\'s declared serving size, not per package. A smaller declared serving size makes every one of those numbers look smaller on the label, without the product itself changing at all: the same bag of chips can show a lower sugar or sodium figure simply by declaring a smaller official serving than most people would eat in one sitting. The Portions & Recommended Amounts research covers the separate finding that larger portions increase how much people eat; this is the related, label-reading half of that same problem: the number on the package and the amount a person consumes are frequently two different quantities, and only checking servings-per-container against how much is eaten reconciles them.',
    citations: [
      {
        source: 'FDA, "How to Understand and Use the Nutrition Facts Label"',
        url: 'https://www.fda.gov/food/nutrition-facts-label/how-understand-and-use-nutrition-facts-label',
  ...
  },
  {
    ...
    summary:
      'US federal law requires nine specific major allergens (milk, eggs, fish, shellfish, tree nuts, peanuts, wheat, soybean, and, since January 1, 2023 under the FASTER Act, sesame) to be disclosed in one of two specific formats: named directly in parentheses right after an ingredient that doesn\'t obviously reveal it ("natural flavor (sesame)"), or grouped together in a plain "Contains" statement immediately after the ingredient list. Sesame\'s 2023 addition is a recent example of how the allergen list itself can change, it took an act of Congress (the FASTER Act, signed 2021) specifically because sesame allergy reactions were not being reliably caught under the eight-allergen list that existed before it. This mechanism exists for exactly the situation an unfamiliar-sounding ingredient name creates: an allergen hiding inside a name that gives no obvious clue on its own.',
    citations: [
      {
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'Three Rules Cover Most of What a Label Says',
    teaser: 'Ingredient order reflects weight, %DV reflects a fixed 2,000-calorie reference, and serving size is the manufacturer\'s choice, knowing these three rules makes almost any label readable.',
    summary:
      'Across every label, three fixed rules do most of the work of reading a label correctly: ingredients are always listed by descending weight (so the first few names matter most), %DV is always calculated against the same fixed 2,000-calorie reference regardless of the product (so 5%/20% is a portable rule of thumb), and serving size is the one number in this whole system a manufacturer gets to define, which is exactly why it deserves its separate check against how much a person eats. None of this requires special training or a nutrition degree, it requires knowing these three fixed rules exist, and reading the label with them in mind rather than trusting the front-of-package marketing to tell the same story.',
    citations: [
      {
        source: 'FDA, "How to Understand and Use the Nutrition Facts Label"',
        url: 'https://www.fda.gov/food/nutrition-facts-label/how-understand-and-use-nutrition-facts-label',
