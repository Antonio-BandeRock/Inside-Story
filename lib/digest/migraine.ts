...
export const MIGRAINE_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    teaser: 'Real, prospective data finds migraine improving in 47% of women by the first trimester, climbing to a real, striking 87% by the third -- the same real estrogen-stability mechanism already named in the menstrual-migraine research, working in reverse.',
    summary: "Migraine offers one of the more genuinely reassuring real pregnancy findings of any condition covered here. A real, prospective diary study found migraine improvement in 47% of women during the first trimester, climbing to 83% by the second, and a real, striking 87% by the third trimester, meaning roughly 9 in 10 pregnant people with migraine experience real improvement by the end of pregnancy. The real, well-understood mechanism connects directly to the already-established menstrual-migraine research: estrogen levels rise to roughly 30-40 times their normal peak menstrual-cycle level by the third trimester, and, critically, STOP cycling entirely, removing the estrogen-withdrawal trigger the research already names as a major real migraine driver. Real research found this improvement most pronounced specifically in people with a pre-existing history of menstrual migraine, the same population most sensitive to estrogen fluctuation in the first place. Worth knowing honestly: not everyone improves, and a real minority report worsening symptoms during pregnancy instead, but the real, dominant pattern across large prospective data is genuine improvement, not the worsening many people might reflexively expect.",
    citations: [
  ...
  // independently verified via WebSearch.
  {
    ...
    summary: "OnabotulinumtoxinA, the same botulinum toxin known by the brand name Botox, is a real, FDA-approved, specifically studied preventive treatment for chronic migraine, worth knowing about directly alongside the CGRP inhibitors already covered in the migraine research. The real, landmark PREEMPT clinical trial program tested this specific treatment in two large, randomized, placebo-controlled trials. PREEMPT 2 found onabotulinumtoxinA statistically significantly superior to placebo for its primary outcome, real reduction in headache days per 28-day period (a real 9.0-day reduction versus 6.7 days for placebo), with real, significant improvement on every secondary measure tested too. Worth knowing honestly: PREEMPT 1, the companion trial, did NOT find a significant difference on its own primary endpoint (headache episodes specifically, a different way of counting than headache days), a real, honest inconsistency between the two trials worth naming directly rather than only citing the positive result. The real, pooled analysis combining both trials' full data did confirm onabotulinumtoxinA as an effective preventive treatment overall, and real research found repeated treatment cycles (administered every 12 weeks) safe and well tolerated over time. Worth knowing directly: this is a real, FDA-approved, evidence-backed option specifically for CHRONIC migraine (15 or more headache days a month, already covered in the episodic-vs-chronic research), not typically used for less frequent episodic migraine, worth discussing directly with a neurologist for anyone whose chronic migraine hasn't responded well to oral preventive medications.",
    citations: [
