...
export const PREGNANCY_FAMILY_PLANNING_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    summary:
      "It's an understandable instinct to wonder whether a daily medication is safe to keep taking while breastfeeding. For levothyroxine specifically, the answer is reassuring enough that major medical guidance treats continuing it as the clearly correct choice. The National Institutes of Health's Drugs and Lactation Database states plainly that limited data on replacement-dose levothyroxine during breastfeeding show no adverse effects in infants, and for a biological reason: levothyroxine (T4) is itself a normal, naturally occurring component of human breast milk already. A treated parent's milk isn't introducing something foreign to a nursing infant's system, just restoring it to a normal level. Thyroid-society guidance goes further, actively recommending that both subclinical and overt hypothyroidism be treated with levothyroxine in women who are nursing. Treatment is the recommended path, not a risk to be weighed against breastfeeding itself. Undertreated hypothyroidism during this period is the actual risk, not the medication used to correct it.",
    citations: [
