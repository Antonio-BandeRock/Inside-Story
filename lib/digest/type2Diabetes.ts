...
export const TYPE_2_DIABETES_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    summary:
      "This category's own already-covered Nauru entry tells a per-capita story, roughly half of one small island's population affected. China and India tell a different kind of story: sheer scale. IDF Diabetes Atlas data finds China carrying the world's largest absolute number of people with diabetes, over 140 million in 2021, projected to exceed 174 million by 2045, with India close behind at nearly 90 million adults, accounting for roughly 1 in every 7 adults with diabetes worldwide. A further complication: over half of everyone with UNDIAGNOSED diabetes globally lives in just three countries, China, India, and Indonesia, meaning the true scale in these countries is likely even larger than the already-enormous confirmed numbers. Projections show India's own diabetes population rising a further 75% by 2050. This is a different, complementary statistic to Nauru's own per-capita story, rather than one country having the 'worst' diabetes problem, global diabetes burden concentrates differently depending on whether the question is which population is proportionally hit hardest (Nauru, the Pacific Islands) or where the largest absolute number of affected people actually live (China, India), both true and both part of the global picture of this disease.",
    chart: {
