...
export const GRAVES_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    summary: "Smoking is well-established as harmful in nearly every way, and for Graves' disease specifically it carries a real, well-documented, dose-dependent risk. A meta-analysis covering 56 studies found smoking (current or past) significantly raises the risk of thyroid eye disease, and an earlier, large meta-analysis found the association even sharper for Graves' ophthalmopathy than for Graves' disease alone. The risk rises with each additional cigarette smoked per day, and even partial smoking reduction appears to lower risk somewhat. Worth naming directly, since it lands differently for different thyroid conditions: the Hashimoto's research documents a real, separate, genuinely counterintuitive finding that smoking tracks with LOWER risk of developing Hashimoto's thyroiditis specifically. The two findings aren't in tension with each other; they're two different diseases with two different real relationships to the same habit. This is one of the clearest, most direct reasons smoking-cessation counseling belongs as a real, named part of Graves' treatment, not just general health advice repeated out of habit.",
    citations: [
  ...
  // WebSearch.
  {
    ...
    summary: "Major life stress is a real, independently studied, and increasingly well-supported environmental trigger for Graves' disease onset, not just a folk belief. A real, current systematic review and meta-analysis (13 studies, 2,892 subjects across nine countries) found stressful life events genuinely associated with Graves' disease onset specifically in people already carrying the genetic susceptibility already covered in the genetic/family-risk research, real evidence this is a gene-environment interaction rather than stress acting alone. The real association was found stronger in samples with more female patients and in younger patients, matching Graves' disease's own real, well-documented demographic pattern. A real, separate, smaller study found newly diagnosed Graves' patients reported significantly more stressful life events in the 12 months before diagnosis than matched controls. Genuinely striking, and worth knowing directly: a real, small study (11 participants) whose Graves' disease developed after acute emotional distress found stress-relief training itself leading to disease remission in that group, a real, if preliminary, hint that addressing the stress response directly might have therapeutic value alongside standard antithyroid treatment, not proof of a stand-alone cure. Worth knowing directly: this connects straight to the already-covered HPA-axis/cortisol research built for other conditions, real, additional evidence that chronic stress management belongs in a real Graves' management plan, not just symptom control through medication alone.",
    citations: [
  ...
  },
  {
    ...
    summary: "This category's own already-covered cardiac-risk research names the real, acute danger of untreated Graves'; real, longer-term data adds an honest, important nuance about what happens AFTER treatment starts working. In one real study, atrial fibrillation affected 72% of patients before antithyroid therapy began, dropping to 25% once treatment was underway, a real, substantial improvement. But treatment success measured by normal thyroid labs doesn't always mean the heart has fully caught up: a real, separate finding shows about 38% of Graves' patients still report real cardiac symptoms months after their thyroid hormone levels have returned to normal. Real research also confirms Graves' reach extends well past the heart alone, into skeletal muscle, the eyes, skin, bone, and liver, the same real, systemic, whole-body pattern the research keeps finding across nearly every condition it covers. Worth knowing directly: a normal TSH doesn't automatically mean every symptom has resolved, and lingering cardiac symptoms after successful treatment are a real, documented pattern worth raising with a doctor rather than assuming they mean the treatment failed.",
    citations: [
  ...
  },
  {
    ...
    summary: "This category's own already-covered remission-rate research names the real overall numbers; real, more specific predictor research answers the practical follow-up question -- who is actually most likely to relapse. Real studies place relapse within the first year after stopping antithyroid drugs at roughly 30-40% overall, with some cohorts finding it as high as 43-50%. Real, independently identified predictors include smoking (the already-covered Graves'-specific smoking research, now with a second real reason to matter), a large goiter, elevated thyroid hormone (FT4) at the START of treatment, stopping the drug before completing a full 12 months, and a low TSH measured just 4 weeks after stopping -- a real, practical early-warning signal a doctor can check for directly rather than waiting to see if symptoms return. Real research has also found a genetic component (variation in immune costimulatory genes and TRAb levels) contributing to relapse risk, tied directly to the already-covered genetic/family-risk research. Worth knowing directly: relapse after stopping treatment is common enough that it should be treated as a real, expected possibility to actively watch for, not a sign that something went wrong with an otherwise successful first round of treatment.",
    citations: [
