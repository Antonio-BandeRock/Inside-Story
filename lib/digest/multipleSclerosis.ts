...
export const MULTIPLE_SCLEROSIS_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    summary: "Research finds autoimmune thyroiditis or subclinical hypothyroidism present in 20% to 25% of people with MS before they start immunomodulatory treatment, a striking figure, meaning roughly one in four to five untreated MS patients also carries measurable thyroid autoimmunity. Hashimoto's thyroiditis specifically shows up as the single most common comorbid thyroid condition in MS cohort studies. The reassuring finding alongside this: thyroid autoimmunity appears to have a neutral effect on the actual course of MS itself, it doesn't seem to make MS worse, it's simply more likely to co-occur. For two practical reasons, first, MS's core immune-cell biology (Th17/Treg imbalance, the same mechanism the Gut & Microbiome research already covers) is a shared thread connecting these two diseases, not a coincidence; second, a regular thyroid panel (see the Hashimoto's self-advocacy content for what to ask for) is arguably even more relevant for someone managing MS than for the general population, given how common this overlap actually is.",
    citations: [
  ...
  },
  {
    ...
    summary: "This category's already-covered latitude gradient (MS rising toward the poles, falling toward the equator) is a well-established pattern, and Sardinia, the same Italian island covered elsewhere for its T1D hotspot, is documented evidence that pattern isn't universal. Sardinia carries a MS prevalence of 361 per 100,000, among the highest rates reported anywhere in the world, despite sitting at a low, Mediterranean latitude where the gradient theory would predict a much lower rate. Research finds this isn't unique to Sardinia specifically, Italy as a whole doesn't fit the expected latitude pattern the way most of the rest of Europe does. The proposed explanation splits into two parts, and researchers state that neither fully resolves it alone: Sardinia's distinct genetic background (already covered in the T1D research on the island) plausibly explains part of the elevated risk, but research states directly that genetic diversity across Italy's different regional populations does NOT explain the dramatic rise in MS across the whole country over the last half-century, pointing instead toward unidentified environmental factors specific to the region. Sardinia is a useful counter-example whenever the latitude gradient gets treated as an absolute rule rather than a strong but imperfect pattern: some populations carry an independently elevated risk that latitude alone can't explain.",
    chart: {
  ...
  // WebSearch.
  {
    ...
    overallTier: 'weak',
    stageNote: 'Directly conflicting findings between an earlier and a more recent study; included specifically for that inconsistency rather than presented as a settled marker.',
