...
export const MULTIPLE_SCLEROSIS_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    teaser: 'A real, specific case of molecular mimicry, the same broad mechanism already covered for gluten and joint tissue, now traced to a virus and a piece of the nerve sheath itself.',
  ...
  },
  {
    ...
    summary: "A meta-analysis pooling over 20,000 cases from case-control and cohort studies found smoking associated with a real, meaningfully increased risk of developing MS in the first place (risk ratio around 1.5, meaning roughly 50% higher risk). What's genuinely less settled is whether smoking also worsens the disease's course once someone already has it. Different meta-analyses have found mixed results on progression to secondary progressive MS specifically, some falling just short of statistical significance, others finding a real, significant association -- real research still working out exactly how strong that second effect is, even though the first (susceptibility) is well established. Worth knowing plainly: there is no protective angle to smoking found here at all, unlike a few other conditions already covered where the real evidence runs the other way.",
    citations: [
  ...
  },
  {
    ...
    teaser: "The corroborating-evidence research already flags this question as unresolved. A large, recent, dedicated MS trial adds real weight to the uncertain side.",
  ...
  },
  {
    ...
    teaser: 'Roughly a quarter of untreated MS patients also carry autoimmune thyroid disease, one of the strongest real comorbidity numbers found across any condition covered here.',
    summary: "Real research finds autoimmune thyroiditis or subclinical hypothyroidism present in 20% to 25% of people with MS before they start immunomodulatory treatment, a genuinely striking figure, meaning roughly one in four to five untreated MS patients also carries real, measurable thyroid autoimmunity. Hashimoto's thyroiditis specifically shows up as the single most common comorbid thyroid condition in MS cohort studies. The real, reassuring finding alongside this: thyroid autoimmunity appears to have a neutral effect on the actual course of MS itself, it doesn't seem to make MS worse, it's simply genuinely more likely to co-occur. Worth knowing directly for two real, practical reasons: first, MS's own core immune-cell biology (Th17/Treg imbalance, the same mechanism the Gut & Microbiome research already covers) is a real, shared thread connecting these two diseases, not a coincidence; second, a regular thyroid panel (see the Hashimoto's self-advocacy content for what to ask for) is arguably even more relevant for someone managing MS than for the general population, given how common this real overlap actually is.",
    citations: [
  ...
  // verified via WebSearch.
  {
    ...
    summary: "Multiple sclerosis is increasingly tied to real, measurable gut microbiome changes, not just symptoms that happen to occur far from the digestive system. Real research across several independent cohorts finds a consistent pattern of gut dysbiosis in MS patients compared to healthy controls, alongside a real, repeated finding of reduced short-chain fatty acids (SCFAs, the same real fermentation byproducts already covered in the gut-microbiome research), with butyrate and propionate specifically showing pronounced anti-inflammatory effects that appear diminished in MS. The single most striking real finding involves one specific organism: Bacteroides fragilis produces a molecule called polysaccharide A that induces regulatory T cells (Tregs) and suppresses neuroinflammation, and when germ-free MS model mice were deliberately colonized with this bacterium, their real, measured disease score improved by stimulating Tregs and suppressing the same Th17 inflammatory pathway already covered in the broader autoimmune gut-health research. A separate, real depletion of Faecalibacterium prausnitzii, another known butyrate producer, has also been linked to impaired gut barrier function and systemic inflammation across several autoimmune conditions, not just MS specifically. Worth knowing directly: this is real, still-developing mechanistic evidence, mostly from animal models and observational human cohorts rather than a completed human intervention trial, but it's a genuine, specific, named example of the gut-immune connection the research already treats as central, now demonstrated concretely inside MS itself.",
    citations: [
  ...
  },
  {
    ...
    summary: "This category's own already-covered latitude gradient (MS rising toward the poles, falling toward the equator) is a real, well-established pattern, and Sardinia, the same real Italian island covered elsewhere for its own T1D hotspot, is real, documented evidence that pattern isn't universal. Sardinia carries a real MS prevalence of 361 per 100,000, among the highest rates reported anywhere in the world, despite sitting at a genuinely low, Mediterranean latitude where the gradient theory would predict a real, much lower rate. Real research finds this isn't unique to Sardinia specifically, Italy as a whole doesn't fit the expected latitude pattern the way most of the rest of Europe does. The real, proposed explanation splits into two parts, and researchers are honest that neither fully resolves it alone: Sardinia's own real, distinct genetic background (already covered in the T1D research on the island) plausibly explains part of the elevated risk, but real research states directly that genetic diversity across Italy's own different regional populations does NOT explain the dramatic real rise in MS across the whole country over the last half-century, pointing instead toward real, unidentified environmental factors specific to the region. Worth knowing directly: Sardinia is a real, genuinely useful counter-example whenever the latitude gradient gets treated as an absolute rule rather than a real, strong but imperfect pattern, some populations carry an independently elevated real risk that latitude alone can't explain.",
    chart: {
