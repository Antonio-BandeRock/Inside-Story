...
export const PSORIASIS_ENTRIES: DigestEntry[] = [
  ...
  // WebSearch.
  {
    ...
    summary: "Psoriasis has a well-documented association with obesity and metabolic syndrome, already relevant given the visceral-fat and adiponectin research covered elsewhere in this app. The adipokine-specific evidence is honestly mixed rather than uniform: several studies found serum adiponectin decreasing and leptin decreasing as psoriasis severity (measured by the PASI score) increased, while other studies found no significant correlation between either hormone and PASI, and at least one more recent study found leptin positively correlated with PASI specifically in female patients. This is : the adiponectin-severity relationship is the more consistently replicated finding across studies, while the leptin-severity relationship specifically remains unsettled, an active area still being worked out rather than a confirmed clinical marker.",
    citations: [
