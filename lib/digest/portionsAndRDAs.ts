...
export const PORTIONS_AND_RDAS_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'Sodium Is the One Nutrient With a Ceiling, Not a Floor',
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'What the Percentage in Insights Means',
    teaser: 'Every amount logged is converted against the RDA/AI/UL tables above. Knowing what those numbers mean is what makes the percentage in Insights worth trusting.',
    summary: 'Every food logged through the builders carries a specific quantity, not a generic "one serving" assumption, and Insights converts that amount against the exact NASEM/NIH ODS reference tables cited throughout these entries to show a percentage of daily need rather than a guess. The value here is reading the resulting percentage correctly: knowing that most nutrients here are floors (aim to reach or exceed), that sodium is a ceiling (aim to stay under), that a single day under an RDA isn\'t automatically a deficiency, and that the RDA itself already carries a safety margin above the bare physiological minimum.',
    citations: [
      {
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'How Many Calories a Body Needs Has a Published Formula Behind It',
    ...
    summary: 'The Mifflin-St Jeor equation estimates Basal Metabolic Rate, the calories a body burns at complete rest, from weight, height, age, and sex, a formula published in 1990 and still the default most clinical dietitians reach for over the older Harris-Benedict equation. Resting energy alone isn\'t the full picture, an inactive day and a physically demanding one burn very different totals from the same resting baseline, so BMR gets multiplied by a Physical Activity Level factor (Sedentary at 1.2 up to Extra Active at 1.9, following the same activity-level framework FAO/WHO/UNU published in 2001) to reach Total Daily Energy Expenditure, the number this app\'s Energy & Portions lens (see Insights) computes live from a person\'s weight, height, age, sex, and Activity Level setting in Profile. This is a different kind of number from the RDA/AI tables in the other portion entries: those are fixed population reference values pulled straight from NASEM\'s tables, while TDEE recomputes as a person\'s weight or activity level changes. TDEE by itself is a maintenance estimate, the calorie total that keeps current weight roughly steady, not a weight-loss or weight-gain target, this app deliberately doesn\'t build a calorie deficit or surplus on top of it yet, given research covered elsewhere in this app around aggressive restriction and thyroid/HPA-axis stress in an autoimmune population.',
    citations: [
      {
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'Protein Scales With Actual Body Weight, Fat and Carbohydrate Split the Rest',
    ...
    summary: 'Protein need scales with a person\'s actual body weight, not a flat percentage of calories, so this app\'s Energy & Portions lens starts there: 0.8 grams per kilogram at Sedentary (the same population RDA covered in the Protein RDA entry above), rising toward 1.0 to 1.2 grams per kilogram at Lightly to Moderately Active (the same range a position paper from the PROT-AGE Study Group found useful for preserving muscle mass), up to 1.6 grams per kilogram at Very or Extra Active (within the 1.4 to 2.0 gram range the International Society of Sports Nutrition\'s position stand names for regularly exercising adults). Whatever calories remain once that protein target is subtracted split between fat and carbohydrate using the midpoint of NASEM\'s Acceptable Macronutrient Distribution Range (fat 20 to 35 percent of calories, carbohydrate 45 to 65 percent), the same 2002/2005 DRI report cited for fiber\'s AI figures in the fiber entry. A produce guide rides alongside these targets too, USDA MyPlate\'s published cup-equivalent amounts (2 cups fruit, 2.5 cups vegetables at the 2,000-calorie reference level) scaled to a person\'s calorie need rather than derived from a calorie percentage, which doesn\'t hold up for foods this calorie-sparse and variable. Where a tracked condition has published staging with a documented protein number attached, that number takes over from the activity-based default instead, currently just Chronic Kidney Disease\'s pre-dialysis-versus-dialysis reversal (see that condition in Life > Conditions).',
    citations: [
      {
