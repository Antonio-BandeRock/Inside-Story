...
export const LUPUS_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    summary:
      "Lupus is often pictured as a young woman's disease, but a meaningful share of cases, 20% by one estimate, are diagnosed at age 50 or later, and this late-onset form has its own distinct clinical shape. Comparative data finds late-onset lupus produces lower disease-activity scores on average, with significantly less fever, fewer of the classic skin and mucous-membrane findings, and fewer positive antibody results than lupus diagnosed earlier in life. The practical risk this creates: late-onset lupus often lacks the textbook butterfly rash and is less likely to involve the kidneys at the outset, instead presenting as pleurisy or pericarditis (inflammation around the lungs or heart), persistent dry eyes and mouth, or joint and muscle aches that get chalked up to ordinary osteoarthritis or general wear and tear. Serologically, late-onset patients are less likely to test positive for anti-dsDNA and RNP antibodies, and more likely to test positive for rheumatoid factor, a pattern that can point evaluation toward the wrong diagnosis. Cohort data does describe a milder overall disease course with less frequent kidney involvement in this later-onset group, honest reassurance that softens, but does not erase, the risk of a slower, harder-to-recognize diagnosis in this age group.",
    citations: [
