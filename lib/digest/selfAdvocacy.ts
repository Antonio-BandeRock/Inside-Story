...
export const SELF_ADVOCACY_ENTRIES: DigestEntry[] = [
  {
    ...
    title: 'A Fuller Lab Panel Usually Has to Be Asked For by Name',
    ...
    summary: "A primary care visit is built around triage rather than a deep investigation. A doctor sees a value in range and, correctly by their training, moves on. That is not negligence. A 15-minute visit with one lab requisition can only cover so much, which is why a fuller panel relevant to a specific chronic condition so often never gets ordered unless the person in the room asks for it by name. Research backs this up: patients who are more \"activated,\" meaning informed, prepared, and an active participant in their care rather than a passive recipient of it, measurably have better outcomes managing chronic conditions. They show up with the right questions already framed. That's the practical skill behind every condition-specific self-advocacy section: not confrontation, just knowing which test to name, why it matters for that condition specifically, and how often asking for it again adds information instead of noise.",
    citations: [
  ...
  },
  {
    ...
    category: 'hashimotos',
    title: 'TPO and Thyroglobulin Antibodies Are What Confirm a Hashimoto\'s Diagnosis',
    teaser: 'TSH tells you the thyroid is underperforming. Antibodies tell you why, and that changes what else is worth watching.',
    summary:
      "A low thyroid on its own doesn't say what's causing it. TPO and thyroglobulin antibodies confirm an autoimmune process is the reason, the specific test Hashimoto's diagnosis is built around (see the Glossary's TPO entry for that history). Clinical guidance notes that elevated TPO antibody titers in someone with only mildly abnormal thyroid numbers (subclinical hypothyroidism) meaningfully raise the odds of progressing to full, overt hypothyroidism, which is why a positive antibody result matters even before TSH itself looks clearly abnormal. Once confirmed, though, retesting antibodies often doesn't change day-to-day treatment the way retesting TSH does. Antibody titers aren't used to adjust a levothyroxine dose, and Hashimoto's antibody levels can stay elevated for a long time without a clean, predictable decline pattern. That makes this an exception to \"more testing is better\": once confirmed, checking once a year at most is enough to notice a long-term trend, rising, falling, or holding steady, without chasing normal week-to-week fluctuation that doesn't mean anything.",
    citations: [
      { source: 'Gaitonde DY, Rowley KD, Sweeney LB 2012: Hypothyroidism: An Update (American Family Physician)', url: 'https://www.aafp.org/pubs/afp/issues/2012/0801/p244.html' },
  ...
  },
  {
    ...
    category: 'hashimotos',
    title: 'Reverse T3: A Test With an Unsettled Answer, Before Asking For It',
  ...
  },
  {
    ...
    category: 'hashimotos',
    title: 'Vitamin D: Commonly Low With Hashimoto\'s',
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'A Magnesium Test Is Reasonable to Order, With One Limitation to Know First',
    teaser: 'The blood test for magnesium checks less than 1% of where the body keeps it, worth asking for anyway, just not over-trusting on its own.',
    summary:
      "This is the one test among these worth requesting with the caveat stated up front, because the caveat is well documented: less than 1% of the body's total magnesium circulates in the blood, which is exactly what a standard serum magnesium test measures. The vast majority sits inside cells and bone, invisible to that one number. A 1998 study demonstrated this directly, finding oral magnesium supplementation could shift the body's magnesium stores with barely any corresponding change in serum levels. That doesn't mean skip the test; a low serum result is still a meaningful finding worth treating. It means a normal serum result doesn't fully rule out a deficiency the way it might for a nutrient measured more sensitively, like vitamin D. Given that limitation, there's little value in retesting frequently expecting the number to track supplementation cleanly. A reasonable approach is checking once as a baseline and again only if new symptoms suggest a problem (muscle cramps, irregular heartbeat, unusual fatigue), not on a fixed schedule.",
    citations: [
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'Ask for Zinc and Copper as a Pair, Especially When Already Supplementing Zinc',
  ...
  },
  {
    id: 'advocacy-cbc',
    category: 'basicHealth',
    title: 'A Complete Blood Count Catches What a Condition-Specific Panel Misses',
    ...
    overallTier: 'moderate',
    relatedIds: ['iron-deficiency-symptoms-staged', 'folate-b12-masking-danger'],
  },
  {
    id: 'advocacy-cmp',
    category: 'hashimotos',
    title: 'Comprehensive Metabolic Panel (CMP): Checking the Organs Doing the Thyroid\'s Hidden Work',
  ...
  },
  {
    ...
    teaser: 'A direct hormonal effect, not a moral failing about diet, and one a 6-month trial showed reverses with treatment.',
    summary:
  ...
  },
  {
    ...
    teaser: "Hashimoto's and type 1 diabetes share enough biology that checking blood sugar isn't an unrelated add-on. It's an adjacent risk.",
    summary: "The Other Autoimmune Diseases category already makes the case that autoimmune conditions don't stay neatly siloed. Hashimoto's and type 1 diabetes both co-occur more than chance would predict, sharing enough underlying immune biology that checking one informs watching for the other. Fasting glucose and hemoglobin A1c (a longer-window average of blood sugar over roughly three months rather than a single-moment snapshot) are the standard way that gets tracked, and the Mitochondria & Metabolism research separately ties visceral fat and insulin resistance directly to Hashimoto's-specific findings, not just general population risk. Neither test needs Hashimoto's-specific extra frequency. Following the general diabetes-screening schedule already agreed with a doctor (typically periodic, more often only if a prior result was borderline or abnormal) is the reasonable approach here, not a reason to add quarterly rechecks with no new symptom or result prompting it.",
    citations: [
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'Fasting Insulin Is an Earlier Warning Sign, and Debated as a Standalone Test',
    teaser: 'Some practitioners treat it as an early-warning test years ahead of an A1c change. The research on how well it predicts that is more mixed than the pitch suggests.',
    summary:
      "The logic behind fasting insulin testing is sound on paper: insulin can start rising before blood sugar itself does, as the body works harder to keep glucose normal, meaning, in theory, it could flag developing insulin resistance years before A1c or fasting glucose would catch it. A 2008 review of methods for assessing insulin sensitivity and resistance laid out exactly this appeal alongside its limitations. Fasting insulin assays aren't as tightly standardized across different labs as glucose or A1c are, and no single, universally agreed cutoff value exists the way one does for A1c. That doesn't make it worthless, but it is a more debated, less standardized test than the other metabolic tests here. It's worth asking about specifically if there's a reason to suspect early insulin resistance (strong family history, visible weight changes, PCOS-type symptoms) rather than requesting it as a routine, no-reason addition to every panel. Not a test to chase repeatedly without a specific concern driving it.",
    citations: [
  ...
  },
  {
    id: 'advocacy-hscrp',
    category: 'basicHealth',
    title: 'Hs-CRP Is a Checkable Inflammation Number, Not One to Chase Every Month',
    teaser: 'The research keeps landing on inflammation as the thread connecting food choices to thyroid hormone. Hs-CRP is how that thread gets measured.',
    summary: "High-sensitivity C-reactive protein is the one lab value that shows up as a checkable outcome behind several of the findings: the Mediterranean-diet and fiber-intake research in Lifestyle & Environment, the IL-6-to-deiodinase-suppression mechanism connecting inflammation directly to lower active thyroid hormone. It's a marker cardiology guidelines recognize as one factor worth weighing in overall risk assessment. hs-CRP is a good baseline to establish once, and worth rechecking deliberately after a sustained change (a months-long shift in diet, a new exercise habit) to see whether it moved. But it isn't a number that meaningfully changes week to week or even month to month for most people, and checking it that often mostly just measures ordinary day-to-day noise. One baseline check, then a deliberate recheck tied to an actual change being tracked, is the reasonable cadence here.",
    citations: [
    ...
    overallTier: 'moderate',
    relatedIds: ['glossary-deiodinase', 'cortisol-chronic-dysregulation-autoimmunity'],
  },
  {
    id: 'advocacy-sex-hormones-menopause',
    category: 'basicHealth',
    title: 'Sex Hormone Panels Are Useful, and Often Not Required to Diagnose Perimenopause Itself',
    teaser: 'A counterintuitive finding: the full hormone panel isn\'t usually what confirms perimenopause is happening.',
    summary:
      "This is the one panel among these where the answer runs against how routinely it tends to get ordered. An influential 2003 clinical review directly asked the question \"is this woman perimenopausal?\" and concluded the most reliable answer usually comes from symptoms and menstrual pattern change, not a single hormone blood draw. Estradiol and FSH in particular fluctuate enough within the same person, cycle to cycle, that one snapshot value can mislead more than it clarifies. That doesn't mean the panel is never useful. It's valuable in specific situations: symptoms that don't fit the usual pattern, ruling out a different cause before starting hormone therapy, or making an actual treatment decision about HRT where a baseline number matters. Perimenopause symptoms (fatigue, mood shifts, temperature sensitivity, sleep disruption) overlap with several chronic conditions' own symptom lists, which is exactly when this panel earns its value: worth asking for specifically when two possible explanations are hard to tell apart, not as a routine, repeat-every-visit addition, since the same day-to-day fluctuation that makes one snapshot unreliable also makes frequent retesting mostly noise.",
    citations: [
  ...
  },
  {
    ...
    teaser: '"Adrenal fatigue" isn\'t a recognized medical diagnosis. A distinct, documented autoimmune condition it gets confused with is: Autoimmune Polyglandular Syndrome Type 2.',
    summary: "Two different things both get called \"cortisol testing,\" and telling them apart matters. A 2025 review states this about as plainly as a medical paper ever does: the terms \"adrenal fatigue,\" \"adrenal asthenia,\" and \"adrenal burnout,\" often attached to mail-order salivary cortisol test kits marketed directly to consumers, \"are not a part of the medical or endocrine lexicon,\" meaning they aren't a diagnosis an endocrinologist would treat as one. That's worth knowing before spending money on one of those kits. What IS worth testing through an actual doctor: the Organs & Body Systems category covers Autoimmune Polyglandular Syndrome Type 2, a documented autoimmune attack on the adrenal glands that can occur alongside Hashimoto's, diagnosed with an AM serum cortisol test or an ACTH stimulation test rather than a saliva kit. It's worth raising directly with a doctor only when there's a reason to (unexplained low blood pressure, unusual skin darkening, severe unremitting fatigue with other red flags), not something to test on a fixed schedule without one of those signs present.",
    citations: [
  ...
  },
  {
    ...
    summary: "Nearly every entry in this category ends the same way: don't retest more than roughly every 6-12 months without a specific reason, because the biology itself doesn't move fast enough for more frequent checks to mean anything beyond ordinary noise. Starting an elimination protocol, the Healing Stages Stage 1, a deliberate, structured change in what's being eaten, is the one legitimate exception to that rule, for a specific reason: it's the single biggest, fastest input change most people ever make to their body chemistry, and the whole point of doing it is to actually see whether it moves something measurable. A sensible approach: get a proper baseline panel (as much of this category's list as practical) right before starting, then one recheck around 8-12 weeks in, long enough for a signal to separate from noise, matching the timelines the gut-repair research already cites for measurable change, but not so frequent that normal week-to-week fluctuation gets mistaken for progress or failure. Outside of a change this deliberate, the standard cadence above still applies. This is a bounded exception, not a license to test constantly during any life change.",
    citations: [],
    overallTier: 'moderate',
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'A Written List Beats a Verbal Request Every Time You Ask a Doctor for a Test',
    teaser: 'The single most effective thing anyone can bring to this conversation isn\'t confidence. It\'s a piece of paper.',
    summary: "Research on patient self-management found something consistently practical: patients who prepare specific, written questions ahead of a visit get more of those questions answered than patients who raise the same concerns verbally, in the moment, under time pressure. The advice is a short, specific, written list (the Bio-Compass symptom log is a ready-made source for exactly this) naming the actual tests relevant to a specific situation, handed over or read from directly rather than described from memory. Framing matters too. \"I'd like to also check my [specific test] this visit, given my [condition]\" lands very differently from an open-ended \"can you test everything.\" Some tests are realistically a same-visit primary-care request (a condition's core panel, CBC, CMP, lipids, vitamin D, B12, ferritin), while others more often need a referral to a specialist or a functional-medicine practitioner to access at all (a more contested or less standardized test, a full sex-hormone panel, a nutrient without a routine lab pathway), a matter of ordering habits and lab access differing by specialty rather than a PCP being unwilling. Insurance coverage is a separate constraint worth asking about directly rather than assuming; some of the less-routine tests may need a specific documented reason to be covered.",
    citations: [
      { source: 'Bodenheimer T, Lorig K, Holman H, Grumbach K 2002: Patient self-management of chronic disease in primary care (JAMA)', url: 'https://pubmed.ncbi.nlm.nih.gov/12435261/' },
  ...
  },
  {
    ...
    category: 'basicHealth',
    title: 'The Prescribing Cascade Can Run in Reverse When Underlying Health Improves',
    teaser: 'One medication\'s side effect, mistaken for a new problem, treated with a second medication, which causes its side effect: a phenomenon with a name in the medical literature.',
    summary: "There's a specific term in the medical literature for a pattern many people with a chronic condition eventually recognize in their medicine cabinet: the prescribing cascade. It begins when a drug's side effect gets misinterpreted as a new medical condition in its own right, leading to a second drug being started to treat what was the first drug's side effect all along, sometimes repeating again from there. It's an established concept in clinical pharmacology, tracing back to a classic 1997 paper on optimizing drug treatment. The hopeful flip side: when an underlying driver of a problem (in the core framing, chronic inflammation and metabolic health) improves, some medications originally started for a downstream symptom can be medically-supervised candidates for reduction. A well-known trial (DiRECT) found that among people who achieved diabetes remission through a structured weight-loss intervention (a mean weight loss of 11.4 kg), 28% remained completely off blood pressure medication at 24 months, a quantified, if partial, success rate. None of this is a case for stopping any medication without medical supervision. It is a reason to ask a doctor directly, as underlying health measurably improves, whether a medication started for a downstream symptom is still needed, rather than assuming a prescription written years ago is permanent by default.",
    citations: [
    ...
    overallTier: 'moderate',
    relatedIds: ['advocacy-how-to-ask', 'glossary-visceral-fat', 'bodyfat-visceral-vs-subcutaneous', 'type2-direct-remission-trial', 'gout-diuretics-prescribing-cascade'],
  },
  {
    id: 'advocacy-fibromyalgia-thyroid-overlap',
    category: 'hashimotos',
    title: 'Fibromyalgia & Hashimoto\'s: A Documented Overlap, Before Accepting Either Label Alone',
    ...
    summary: "Fibromyalgia and Hashimoto's share enough symptom overlap (fatigue, muscle pain, brain fog) that untangling which is actually driving a given symptom is a legitimate clinical challenge, not a simple either/or. Research backs up that the overlap runs deeper than shared symptoms alone: several studies have found autoimmune thyroiditis and thyroid antibodies present in an elevated share of fibromyalgia patients compared to the general population, across independent research groups. One specific finding: a study examining thyroid hormone treatment among fibromyalgia patients found 34% (33 of 103) were being treated with thyroid hormone, a high enough number that the study's title characterizes it as \"excess use.\" That doesn't mean thyroid treatment is wrong for any individual person with both diagnoses; some have both independent conditions. But it's a direct reason a fibromyalgia diagnosis is worth pairing with the full thyroid panel the Self Advocacy category already recommends, rather than either diagnosis fully explaining away symptoms that might actually belong to the other, or reflect an overlap of both at once.",
    citations: [
