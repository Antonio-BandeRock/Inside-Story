...
export const SELF_ADVOCACY_ENTRIES: DigestEntry[] = [
  ...
  },
  {
    ...
    summary:
      "A CBC isn't a Hashimoto's-specific test, and that's exactly its value here. It's the general-purpose net that catches the related problems already covered above (iron-deficiency anemia, B12-deficiency anemia from co-occurring autoimmune gastritis) without needing to separately diagnose each one from scratch. Red blood cell size and shape on a CBC can point toward which specific deficiency to chase down first, before ever ordering the more specific iron or B12 panels above, an efficient first step rather than a redundant one. It's also simply part of a complete picture of overall health, catching things entirely unrelated to Hashimoto's that are still worth knowing about. As part of an annual physical or a periodic full-panel check, a CBC is reasonable to include every time. There's little value asking for it more often outside of actively investigating a new, specific symptom (unusual bruising, persistent unexplained fatigue, signs of infection) that would justify a sooner recheck on its own.",
    citations: [],
    overallTier: 'moderate',
