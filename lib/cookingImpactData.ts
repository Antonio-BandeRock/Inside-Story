// Cited data behind Insights' "Cooking Impact" lens, 2026-08-10.
// Answers a different question than the reference database's
...
// builder's free-standing "Cook Prep" ingredient field (stored as
// descriptive metadata only, never read by any nutrient/scoring query:
// see each builder's MealIngredientInput-style types). Neither of
...
// logged ingredient. This is a standalone, informational reference, the
// same non-gating, doesn't-touch-tracked-totals shape as the alcohol
// calculator (components/AlcoholCalculator.tsx) and the header comment in
// lib/alcoholCalculator.ts on why that boundary matters.
//
// Deliberately NOT a single formula the way ethanol calorie math is.
// Nutrient degradation does not reduce to one clean equation
...
// per-compound reference table instead, each entry marked by
// confidence:
//   'measured'  a cited study measured this exact combination.
//   'reasoned'  no dedicated study found for this exact combination;
//               an inference from the SAME compound's cited
//               mechanism (leaches into water / heat-stable
//               / enzyme-dependent), not a guess with nothing behind it.
//   'baseline'  the raw/uncooked reference point everything else in
//               that compound's row is compared against.
//
...
// elsewhere in this app (SUB_CRITERION_SOURCES and selectPrepTips in
// lib/sixDimensionsReference.ts), reused directly rather than
...
// primary papers (not a secondary aggregator) before being written
// in here.
...
// Ordered by water/heat exposure, "Raw" first as the baseline
// every other row is compared against, matching this app's standing
// "an answer, not an absent one" convention for a first list entry.
...
export const COOKING_IMPACT_COMPOUNDS: CookingImpactCompound[] = [
  {
    ...
    byMethod: [
      { methodId: 'raw', confidence: 'baseline', summary: 'Full amount present: the reference point every other row is measured against.' },
      {
        methodId: 'steamed',
        confidence: 'measured',
        summary: "True retention ranged 0-89% across the vegetables tested: generally the best of the wet-heat methods, since there's little direct water contact.",
      },
      {
        methodId: 'microwaved',
        confidence: 'measured',
      ...
      },
      {
        ...
        confidence: 'measured',
        summary: 'True retention ranged 0-74% across the vegetables tested: the worst method tested, since vitamin C leaches into the water and that water is then poured away.',
      },
      {
  ...
  },
  {
    ...
    byMethod: [
      { methodId: 'raw', confidence: 'baseline', summary: 'Full amount present.' },
      {
        methodId: 'steamed',
        confidence: 'measured',
        summary: 'No significant loss measured for spinach (up to 4.5 min) or broccoli (up to 15 min): steaming has minimal direct water contact, and leaching is the mechanism that matters here.',
      },
      {
        methodId: 'microwaved',
        confidence: 'reasoned',
      ...
      },
      {
        methodId: 'boiled_kept',
        confidence: 'reasoned',
        summary: 'The same leaching, but consuming the liquid (soup, stew) keeps most of what leached out in the meal. Not directly tested.',
      },
      {
        methodId: 'baked_roasted',
        confidence: 'reasoned',
        summary: "No direct water contact: likely closer to steaming's retention than boiling's, but not directly measured.",
      },
      {
        methodId: 'sauteed',
        confidence: 'reasoned',
  ...
  },
  {
    ...
    citation:
      "Felker, Bunch & Leung, \"Concentrations of thiocyanate and goitrin in human plasma...,\" Nutrition Reviews, 2016;74(4):248-58, PMID 26946249. The same citation already behind this app's Goitrogenic Load scoring. Meaningful interference generally requires large intakes plus pre-existing iodine insufficiency.",
    byMethod: [
      {
        methodId: 'raw',
        confidence: 'baseline',
        summary: "Full goitrogenic effect present: this is the one condition this app's Goitrogenic Load dimension flags.",
      },
      { methodId: 'steamed', confidence: 'measured', summary: 'Cooking (steaming, boiling, or roasting) deactivates roughly 90% of the effect.' },
      { methodId: 'microwaved', confidence: 'measured', summary: 'Same roughly 90% reduction as any cooking method.' },
      { methodId: 'boiled_drained', confidence: 'measured', summary: 'Same roughly 90% reduction as any cooking method.' },
      { methodId: 'boiled_kept', confidence: 'measured', summary: 'Same roughly 90% reduction as any cooking method.' },
      { methodId: 'baked_roasted', confidence: 'measured', summary: 'Same roughly 90% reduction as any cooking method.' },
      { methodId: 'sauteed', confidence: 'measured', summary: 'Same roughly 90% reduction as any cooking method.' },
      { methodId: 'grilled_broiled', confidence: 'measured', summary: 'Same roughly 90% reduction as any cooking method.' },
    ],
  },
  {
    id: 'lectins',
    ...
    citation:
      "\"Assessment of Lectin Inactivation by Heat and Digestion,\" PMID 21374488. The same citation already behind this app's Lectins (Legumes) scoring.",
    byMethod: [
      { methodId: 'raw', confidence: 'baseline', summary: 'Full lectin content present: raw or undercooked legumes can irritate the gut.' },
      {
        methodId: 'boiled_drained',
        confidence: 'measured',
        summary: 'A long soak-and-boil (not just a quick rinse or a light simmer) is needed to fully deactivate lectins. Undercooked beans stay a risk even after some heat.',
      },
      {
        methodId: 'boiled_kept',
        confidence: 'measured',
        summary: 'Same requirement: a long boil, not just a brief simmer, whether or not the liquid is kept.',
      },
      {
        methodId: 'steamed',
        confidence: 'reasoned',
        summary: 'Steaming alone is not the tested method here: legumes need a boil, not steaming, to reliably deactivate lectins.',
      },
      { methodId: 'microwaved', confidence: 'reasoned', summary: 'Not the tested method: a boil is the established, reliable approach for legumes specifically.' },
      {
        methodId: 'baked_roasted',
        confidence: 'reasoned',
        summary: 'Not the tested method for raw legumes: a soak-and-boil first is still the reliable approach.',
      },
      {
        methodId: 'sauteed',
        confidence: 'reasoned',
        summary: 'Brief, direct heat alone is not established as reliable for legume lectins: a soak-and-boil first is the tested approach.',
      },
  ...
  },
  {
    ...
    mechanism: 'Minerals are elements, so heat itself cannot destroy them. Loss only happens through leaching into cooking water that then gets discarded.',
    citation: "Established food-science mechanism, not one specific study: minerals cannot be broken down by heat the way a vitamin molecule can.",
    byMethod: [
      ...
      { methodId: 'boiled_drained', confidence: 'reasoned', summary: 'A meaningful portion leaches into the water, and is then lost when that water is poured away.' },
      {
        methodId: 'boiled_kept',
        confidence: 'reasoned',
        summary: 'The same leaching happens, but nothing is lost if the liquid is eaten too (a soup, a braise): minerals stay in the meal.',
      },
  ...
  },
  {
    ...
    byMethod: [
      ...
      { methodId: 'baked_roasted', confidence: 'reasoned', summary: 'Generally well retained, though very high heat over a long time can start to degrade them.' },
      {
        methodId: 'sauteed',
        confidence: 'reasoned',
        summary: 'Generally well retained, and cooking with a fat can improve absorption, since these vitamins need dietary fat present to absorb well in the first place.',
      },
