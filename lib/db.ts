...
export async function saveMealFavorite(payload: {
  ...
  notes?: string;
  components: MealFavoriteComponent[];
  // True when this row exists only to carry a scheduled meal's components, not
  // because anyone chose to keep it. Such rows stay fully usable by everything
  // that resolves a favorite BY ID (getMealFavorite, a scheduled meal resuming
  // its components); they are simply never listed as favorites.
  ...
  await db.runAsync(
    `
      INSERT INTO favorites (id, item_type, name, payload_json, last_used_at, created_at, updated_at, auto_generated)
      VALUES (?, 'meal', ?, ?, ?, ?, ?, ?)
    `,
    id,
    fullPayload.name,
...
// exact error text -- traced at the time to a known native race between
// getGroundThemeSync()'s own second, synchronous SQLite connection to
// this same DB_NAME file and getDatabase()'s own async one. That second
// connection is gone now (see getGroundThemeSync's own header comment in
// lib/visualPreferences.ts, rebuilt the same day to read a plain mirror
// file instead, after this same symptom recurred under a different
// native function name literally hours after the retry below first
// shipped) -- the ONE confirmed real cause of this error in this app is
// fixed at the source, not just retried around. This retry stays in
// place as a general resilience measure against any other, rarer trigger
// of the same underlying native error class, not as the primary fix
// anymore: if the very first real statement this app issues against the
// main db fails with this specific, unambiguous native signature, the
...
async function runDatabaseInitialization() {
  ...
  try {
    await db.execAsync(`
      ...
      -- WHICH CURATED RECIPES A SCHEDULED MEAL WAS BUILT FROM, 2026-09-15.
      --
      -- scheduleMealPlanSlot resolves a plan slot's curated recipes into saved
      -- dishes and, until now, threw the recipe ids away: the dishes keep the
      -- ingredients and the steps but carry no pointer back to what they came
      -- from. That was fine while a plan only ever had to be cooked. It is not
      -- fine now that a plan has to be SENT, because lib/partnerSync.ts's whole
      -- reason for being small is that a day is a handful of curated recipe ids
      -- resolved against a reference database both phones already have.
      --
      -- Deliberately a record of what was scheduled, written at the moment it is
      -- scheduled, rather than something derived afterwards. Reconstructing
      -- "which recipe was this?" by matching a saved dish's name back against
      -- curated_recipes would be guessing, and a plan that resolves to the wrong
      -- dish on somebody else's phone is worse than sending no plan at all.
      --
      -- schedule_item_id is what keeps this honest without any cleanup code:
      -- getMealPlanForSync joins through it and only reports slots whose meal is
      -- still on the schedule as 'planned'. A meal deleted, eaten, or skipped
      -- drops out of the next send on its own, so a partner is never shown a day
      -- that stopped being true here.
      ...
      -- WHAT A PARTNER SENT, 2026-09-15.
      --
      -- Separate from meal_plan_slots above on purpose. Their plan is a copy of
      -- something another person owns, it never becomes a scheduled meal here,
      -- and nothing in this app should be able to confuse the two. Keeping both
      -- in one table with an owner column would put exactly one WHERE clause
      -- between a partner's dinner and somebody's own schedule.
      --
      -- Replaced wholesale per partner on each arrival, never merged: what they
      -- last sent is their current plan, the same reading setPartnerConditionCodes
      -- already applies to their condition list, and a day they dropped has to be
      -- able to disappear.
      CREATE TABLE IF NOT EXISTS partner_meal_plan_slots (
        id TEXT PRIMARY KEY,
        ...
        received_at TEXT NOT NULL DEFAULT (datetime('now')),
        UNIQUE (connection_id, date, meal_type)
      );
      ...
      CREATE TABLE IF NOT EXISTS grocery_list_items (
        ...
        note TEXT,
        -- Amounts that could not be added to the main one (a weight and a
        -- volume of the same food), and the scheduled meals this line is for.
        -- JSON because both are short lists belonging to one row, never
        -- queried across rows.
    ...
    const groceryItemColumns = await db.getAllAsync<{ name: string }>('PRAGMA table_info(grocery_list_items)');
    if (groceryItemColumns.length > 0) {
      if (!groceryItemColumns.some((column) => column.name === 'extra_amounts_json')) {
        await db.execAsync('ALTER TABLE grocery_list_items ADD COLUMN extra_amounts_json TEXT;');
      }
    ...
    // Confirmed, and wider than meal plans. A scheduled meal remembers which
    // components to resume with through schedule_items.source_favorite_id, so
    // EVERY scheduling path in the app has always created a favorite to point
    // at, deliberately and independent of any "also save as a favorite" choice
    // (see MealBuilder's own comment on this: "structurally required, not a
    // preference"). That reasoning is sound; what was wrong is that a row
    // created only to carry components then showed up everywhere real
    // favorites are listed. A 6-week plan is 126 slots, so it buried them.
    //
    // The carrier mechanism is left exactly as it is. This flag only separates
    // "a favorite someone chose to keep" from "a row holding a scheduled
    // meal's components", and listFavorites filters on it.

    let autoGeneratedColumnsJustAdded = false;
    const favoriteColumns = await db.getAllAsync<{ name: string }>('PRAGMA table_info(favorites)');
    const hasAutoGeneratedColumn = favoriteColumns.some((column) => column.name === 'auto_generated');

    if (!hasAutoGeneratedColumn) {
      await db.execAsync('ALTER TABLE favorites ADD COLUMN auto_generated INTEGER NOT NULL DEFAULT 0;');
    }

    // 2026-09-21, found the first time this script ran against a brand-new
    // database (the desktop build's first launch): the one-time pass below
    // reads schedule_items.source_favorite_id, and that column is added
    // further down this same function, so on a fresh install the UPDATE
    // failed with "no such column" and initialization stopped here. Every
    // phone this has run on already had the column from an earlier version,
    // which is why it never showed. A fresh database has no favorites to
    // mark, so the pass now runs only when the column is already there.
    const scheduleItemsHaveSourceFavorite = (
      await db.getAllAsync<{ name: string }>('PRAGMA table_info(schedule_items)')
    ).some((column) => column.name === 'source_favorite_id');

    if (!hasAutoGeneratedColumn && scheduleItemsHaveSourceFavorite) {
      // One-time pass over favorites that already exist. A carrier favorite is
      // created moments before the schedule item that points at it, so the two
      // timestamps are seconds apart. A favorite someone made deliberately and
      // scheduled later (the Meals lens can point at an existing one) is
      // minutes, hours or days older, and is left alone. Five seconds is
      // generous for the gap between the two writes and far tighter than any
      // realistic "made it, then scheduled it" gap.
      await db.execAsync(`
        UPDATE favorites SET auto_generated = 1
        WHERE id IN (
          SELECT si.source_favorite_id
          FROM schedule_items si
          JOIN favorites f ON f.id = si.source_favorite_id
          WHERE si.source_favorite_id IS NOT NULL
            AND abs(julianday(f.created_at) - julianday(si.created_at)) * 86400 <= 5
        );
      `);
    }

    // Every saved dish whose only reason for existing is a carrier favorite.
    // Done in JS rather than SQL because the link lives inside the favorite's
    // own payload_json, and reading it here needs no assumption about whether
    // this build's SQLite has the JSON1 extension compiled in.
    //
    // Deliberately keyed off favorites already marked auto_generated above,
    // so a dish that belongs to a favorite the person actually made is never
    // touched. Runs on the same pass that adds the columns, so it happens once.
    if (autoGeneratedColumnsJustAdded) {
      const carriers = await db.getAllAsync<{ payload_json: string }>(
        'SELECT payload_json FROM favorites WHERE auto_generated = 1',
      );
      const idsByTable = new Map<string, string[]>();
      for (const carrier of carriers) {
        let parsed: { components?: { componentType?: string; componentId?: string }[] } | null = null;
        try {
          parsed = JSON.parse(carrier.payload_json);
        } catch {
          continue;
        }
        for (const component of parsed?.components ?? []) {
          const table = component.componentType
            ? COMPONENT_TABLE_BY_TYPE[component.componentType as MealComponentType]
            : undefined;
          if (!table || !component.componentId) continue;
          if (!idsByTable.has(table)) idsByTable.set(table, []);
          idsByTable.get(table)!.push(component.componentId);
        }
      }
      for (const [table, ids] of idsByTable) {
        // Chunked: a 6-week plan can reference well over a hundred rows in one
        // table, and SQLite has a real limit on bound parameters per statement.
        for (let start = 0; start < ids.length; start += 200) {
          const chunk = ids.slice(start, start + 200);
          const placeholders = chunk.map(() => '?').join(', ');
          await db.runAsync(`UPDATE ${table} SET auto_generated = 1 WHERE id IN (${placeholders})`, ...chunk);
        }
      }
    }

    // 2026-08-30, direct report: "I think there are some automatically saved
    // sides, Salads and Bowls, and Fermentations, and Beverages, and Snacks,
    // and Soups, and Handhelds, and probably the others will show up too."
    // Correct, and the same shape as the favorites problem fixed earlier the
    // same day, one level down. scheduleMealPlanSlot turns each curated recipe
    // in a slot into a real saved dish via saveComponentFromCuratedPayload,
    // because a scheduled meal resolves its components from real saved records.
    // That is structurally required. What was wrong is that those records then
    // appeared in every "Saved Sides"/"Saved Salads" list as though someone had
    // built them. A 6-week plan is 126 slots, most with more than one component.
    //
    // Same treatment as favorites: the record stays and stays resolvable by id,
    // it is simply never listed as something the person made.
    for (const table of Object.values(COMPONENT_TABLE_BY_TYPE)) {
      const columns = await db.getAllAsync<{ name: string }>(`PRAGMA table_info(${table})`);
      if (!columns.some((column) => column.name === 'auto_generated')) {
        await db.execAsync(`ALTER TABLE ${table} ADD COLUMN auto_generated INTEGER NOT NULL DEFAULT 0;`);
        autoGeneratedColumnsJustAdded = true;
      }
    }
...
// 2026-08-30: auto_generated = 0 excludes dishes that exist only to carry a
// scheduled meal's components (see saveComponentFromCuratedPayload). They are
// still fully resolvable by id, so every scheduled meal still rebuilds
// correctly; they are simply not things the person made, so they do not belong
// in a list of things the person made.
...
export async function listFavorites(
  ...
  return db.getAllAsync<FavoriteRecord>(
    `
      ...
      FROM favorites
      WHERE auto_generated = 0${itemType ? ' AND item_type = ?' : ''}
...
async function saveComponentFromCuratedPayload(
  ...
  // True when this dish exists only so a scheduled meal can resolve its
  // components, not because anyone chose to build it. Such records stay fully
  // usable by everything that reads them BY ID; they are simply never listed
  // among the things the person made.
  autoGenerated = false,
): Promise<{ id: string }> {
  // Every one of the 11 saveX() functions requires an instructions array
  // (never undefined) while BuilderFavoritePayload's field is optional.
  // Since 2026-08-26 getCuratedRecipe() fills it from the curated_recipes
  // instructions column (backfilled for every recipe from the Digest's
  // authored steps), so a dish saved from a system recipe carries its
  // steps, and a meal that holds it shows them on Today's Meals through
  // resolveMealComponent. The fallback to [] is for a payload from
  // somewhere else that has none, not the normal case. (An earlier version
  // of this comment said the column did not exist; it did not, until that
  // day.)
  const withInstructions = { ...payload, instructions: payload.instructions ?? [] };
  const saved = await saveComponentByBuilderType(builderType, withInstructions);
  if (autoGenerated) {
    const db = await getDatabase();
    const table = COMPONENT_TABLE_BY_TYPE[builderType as MealComponentType];
    if (table) await db.runAsync(`UPDATE ${table} SET auto_generated = 1 WHERE id = ?`, saved.id);
  }
  return saved;
}

async function saveComponentByBuilderType(
  builderType: BuilderFavoriteItemType,
  withInstructions: BuilderFavoritePayload & { instructions: string[] },
): Promise<{ id: string }> {
  switch (builderType) {
    case 'side':
...
// Records which curated recipes a scheduled slot was built from.
//
// Lives here, beside the scheduling itself, because this is the only moment
// the recipe ids and the resulting schedule row both exist. The rest of the
// mechanism (reading it back, holding a partner's copy) is lib/mealPlanSync.ts;
// this half cannot be, because that module imports from this one.
//
// Keyed on (date, meal_type) and upserted, matching how the schedule itself
// behaves: one breakfast per day, and re-planning a day replaces what was
// there rather than stacking a second record beside it.
async function recordMealPlanSlot(
  date: string,
  mealType: 'breakfast' | 'lunch' | 'dinner',
  recipeIds: string[],
  scheduleItemId: string,
): Promise<void> {
  const clean = recipeIds.filter((id) => typeof id === 'string' && id.trim().length > 0);
  if (clean.length === 0) return;
  const db = await getDatabase();
  await db.runAsync(
    `INSERT INTO meal_plan_slots (id, date, meal_type, recipe_ids_json, schedule_item_id)
     VALUES (?, ?, ?, ?, ?)
...
async function scheduleMealPlanSlot(
  ...
  for (const ref of refs) {
    ...
    const saved = await saveComponentFromCuratedPayload(ref.builderType, recipe, true);
  ...
  // autoGenerated: nobody asked for 126 favorites by setting up a 6-week plan.
  // This row exists so the scheduled meal can resume its components later.
  const favorite = await saveMealFavorite({ name: title, mealType, components, autoGenerated: true });
  const scheduleItemId = await scheduleMeal({ title, mealType, scheduledFor, sourceFavoriteId: favorite.id });

  // The curated recipe ids, kept rather than discarded, 2026-09-15. The saved
  // dishes above carry the ingredients and the steps but no pointer back to
  // what they were built from, and a plan that travels to a partner is nothing
  // but those pointers. refs is already in slot order, so the record matches
  // what somebody would read down the page.
  await recordMealPlanSlot(
    scheduledFor.slice(0, 10),
    mealType,
    refs.map((ref) => ref.curatedRecipeId),
    scheduleItemId,
  );
...
export type ShoppingListItem = {
  category: string;
  // The raw, purchasable identity: the reference database's base_name, never
  // a prep variant.
  //
  // 2026-09-01, direct instruction: "Prep variants are for the recipes, not
  // for purchasing raw ingredients." This used to carry short_name, which
  // encodes preparation ("Broccoli (boiled)"), with two consequences. It told
  // someone to buy a thing no store sells, and because the list deduplicated
  // on that name, one head of broccoli roasted in a side and shredded raw
  // into a salad came out as two separate lines. 358 foods in this database
  // carry more than one prep variant, so that was not a rare case.
  //
  // The prep variant still matters, and still lives on the recipe and on the
  // saved dish, where it drives nutrients and scoring. It has no business on
  // a shopping list.
  foodName: string;
  unit: string;
  quantity: number;
  // Amounts that genuinely could not be folded into the one above. Weights
  // add to weights and volumes add to volumes with no knowledge of the food,
  // but crossing between them needs a density this app does not have for most
  // foods, so both are shown rather than one being dropped or guessed.
...
// One line of one meal's requirement, before anything is added together.
...
async function shoppingListItemsForFavorite(favoriteId: string, mealName: string): Promise<RawShoppingEntry[]> {
  const favorite = await getMealFavorite(favoriteId);
  if (!favorite) return [];
  const items: RawShoppingEntry[] = [];
...
async function shoppingListItemsForMeal(mealId: string, mealName: string): Promise<RawShoppingEntry[]> {
...
// Every distinct food's raw purchasable name in one pass over the reference
// database, rather than one query per ingredient. A week of meals resolves
// well over a hundred ingredients, and this list is built while someone waits
// for it, so an N+1 here would be felt.
//
// A food that cannot be resolved keeps whatever name the saved dish stored,
// which is an honest fallback: it is still the right thing to buy, just
// possibly still carrying its prep word.
async function resolvePurchasableNames(entries: RawShoppingEntry[]): Promise<Map<string, string>> {
  const byFoodId = new Map<string, string>();
  const referenceIds = new Map<string, { id: number; source: string }>();
  for (const entry of entries) {
    if (!entry.foodId || byFoodId.has(entry.foodId) || referenceIds.has(entry.foodId)) continue;
    const [idPart, source] = entry.foodId.split('|');
    const numericId = Number(idPart);
    if (!source || Number.isNaN(numericId)) continue;
    // A scanned product is already named the way it is sold, and lives
    // outside the reference database entirely.
    if (source === 'Scanned') continue;
    referenceIds.set(entry.foodId, { id: numericId, source });
  }
  if (referenceIds.size === 0) return byFoodId;

  const db = await getReferenceDatabase();
  const values = Array.from(referenceIds.values());
  // Chunked because one shopping list can reference well past SQLite's own
  // parameter ceiling, the same care getUpcomingShoppingList's own callers
  // already take elsewhere in this file.
  const CHUNK = 200;
  for (let start = 0; start < values.length; start += CHUNK) {
    const chunk = values.slice(start, start + CHUNK);
    const placeholders = chunk.map(() => '(food_id = ? AND source = ?)').join(' OR ');
    const params = chunk.flatMap((value) => [value.id, value.source]);
    const rows = await db.getAllAsync<{ foodId: number; source: string; baseName: string }>(
      `SELECT food_id AS foodId, source, base_name AS baseName FROM foods WHERE ${placeholders}`,
      ...params,
    );
    for (const row of rows) {
      byFoodId.set(`${row.foodId}|${row.source}`, row.baseName);
    }
  }
  return byFoodId;
}
...
export async function getUpcomingShoppingList(daysAhead: number = 4): Promise<ShoppingListSection[]> {
  ...
  for (const entry of allEntries) {
    ...
    const byName = grouped.get(entry.category)!;
    const key = purchasable.trim().toLowerCase();
    const existing = byName.get(key);
    if (existing) {
...
export type ReminderCandidate = {
  ...
  // themselves as units per day of a labelled serving instead (see the
...
// Every curated recipe, grouped the way the Digest's own Recipes category
// already groups them (by which builder they open into), for the meal picker's
// "system meals" section.
//
// 2026-08-30, direct steer: "this Find a Meal You've Had is mislabeled because
// they could want to find a meal they haven't had yet. It should also have
// access to the system meals generally in an order that makes sense." One query
// rather than eleven calls to listCuratedRecipes, since the picker wants all of
// them at once; the builder type comes back on each row so the caller can
// section them without a second lookup.
export type CuratedRecipeListRow = CuratedRecipeSummary & { builderType: BuilderFavoriteItemType };

export async function listAllCuratedRecipes(): Promise<CuratedRecipeListRow[]> {
  const db = await getReferenceDatabase();
  const rows = await db.getAllAsync<{
    id: string;
    name: string;
    ...
    'SELECT id, name, flavor_profile, health_benefit, builder_type FROM curated_recipes ORDER BY builder_type, sort_order',
  );
  return rows.map((row) => ({
    id: row.id,
    name: row.name,
    flavorProfile: row.flavor_profile,
    healthBenefit: row.health_benefit,
    builderType: row.builder_type as BuilderFavoriteItemType,
  }));
}

// Turns one curated recipe into a real saved dish and logs it as a meal.
//
// A curated recipe is reference content shared by everyone, so it cannot be
// logged directly: it has to become one of this person's own saved dishes
// first, which is exactly what "Build This Recipe" already does in every
// builder. This is that same step, without making someone walk through the
// builder to take it.
export async function logCuratedRecipeAsMeal(input: {
  recipeId: string;
  mealType: string;
  eatenAt: string;
  photoUri?: string | null;
}): Promise<{ id: string } | { error: string }> {
  const recipe = await getCuratedRecipe(input.recipeId);
  ...
  const created = await createMealFromComponents({
    ...
    eatenAt: input.eatenAt,
    isImmediate: true,
    components: [{ componentType: builderType, componentId: saved.id, yourSharePercent: 100 }],
  });
  if ('error' in created) return created;
  if (input.photoUri) {
    const db = await getDatabase();
    await db.runAsync('UPDATE meals SET photo_uri = ? WHERE id = ?', input.photoUri, created.id);
  }
  return { id: created.id };
}

// Same conversion, but placed on the schedule instead of logged. The carrier
// favorite is marked auto-generated for the same reason every other scheduling
// path's is: it exists so the scheduled meal can resume its components, not
// because anyone chose to keep it.
export async function scheduleCuratedRecipe(input: {
  recipeId: string;
  mealType: string;
  scheduledFor: string;
...
async function getCuratedRecipeBuilderType(recipeId: string): Promise<BuilderFavoriteItemType | null> {
  const db = await getReferenceDatabase();
  const row = await db.getFirstAsync<{ builder_type: string }>(
    'SELECT builder_type FROM curated_recipes WHERE id = ?',
    recipeId,
  );
