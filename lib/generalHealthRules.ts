...
const COFFEE_RULE: GeneralHealthRule = {
  ...
  message: `If you take levothyroxine, coffee timing matters more than roast or bean choice. A pharmacokinetic study found espresso taken together with a levothyroxine dose delayed peak absorption by 38-43 minutes and reduced it by 19-36% (Benvenga et al., cited in a systematic review of levothyroxine/food interactions, PMC8002057): coffee polyphenols and tannins can bind the medication before it's absorbed, and caffeine speeds gut transit, giving it less time to dissolve. Coffee taken about an hour after a dose showed no effect. A separate study found liquid levothyroxine solution wasn't affected even 5 minutes after coffee (Endocrine Society, 2022), worth asking your doctor about if timing is a recurring problem for you.

Filtered vs. unfiltered is well-documented (Harvard T.H. Chan School of Public Health; clinical trials summarized in American Heart Association statements): unfiltered coffee (French press, Turkish, boiled, some espresso) retains oily compounds called cafestol and kahweol, which suppress the liver's LDL clearance. Studies put the effect at roughly 10-16 mg/dL higher LDL cholesterol compared to filtered coffee. A paper filter traps nearly all of it.

Roast level and bean variety matter too, but this app's database doesn't track either one separately for any coffee item on file, so this is general knowledge, not something scored on the specific food you picked:
- Lighter roasts retain more chlorogenic acids (antioxidant compounds broken down by heat: roasting can destroy up to ~90% of them by the time a bean reaches a dark roast). Dark roasts develop more melanoidins instead, the compounds responsible for the color, which carry antioxidant and prebiotic properties of their own. It's a tradeoff, not a clear "better" direction.
...
const JUICE_RULE: GeneralHealthRule = {
  ...
  message: `Straight juice removes the fiber that would normally slow sugar absorption and limit how much you drink in one sitting: the juice of four oranges takes seconds to drink; eating four oranges doesn't. At the population level, this shows up in long-term data: a large study across three prospective cohorts found daily fruit juice intake tracked with up to 21% higher type 2 diabetes risk, while whole fruit, especially blueberries, grapes, and apples, tracked with lower risk (Muraki et al., 2013, BMJ). A separate population study (the Maastricht Study, Diabetes Care, 2022) found fructose from fruit juice and sugar-sweetened drinks independently associated with more fat stored in the liver.

The simple "juice always spikes your blood sugar faster than whole fruit" claim doesn't hold up as a guarantee: a 2025 randomized trial in adults with type 2 diabetes found no measurable difference in glucose or insulin response between orange juice and whole orange pieces when both were eaten as part of a meal with matched sugar content. The bigger real-world risk is likely portion size and how easy juice makes it to drink a lot of sugar quickly, not a fundamentally different absorption curve every time.

For Hashimoto's specifically, sugar swings are worth caring about, but two pieces of the reasoning need a correction. Blood sugar instability is linked to more inflammatory immune activity: unstable glucose can push T cells toward more inflammatory signaling patterns implicated in autoimmune flares. But it's the CRASH after a sugar spike, not the spike or insulin release itself, that triggers a stress-hormone response (cortisol, along with adrenaline) as your body works to bring glucose back up. Cortisol, in turn, is well-documented to suppress the enzymes that convert inactive thyroid hormone (T4) into active T3, while favoring the pathway that makes inactive reverse T3 instead: documented physiology, but this specific chain (juice -> sugar crash -> cortisol -> measurably less active thyroid hormone) hasn't been directly tested as an outcome of drinking juice; it's a plausible chain built from separately-established pieces, not a proven one.

Gut permeability ("leaky gut") shows up in published research too: fructose specifically has been linked to disrupted tight-junction proteins in the gut lining in animal studies, and "leaky gut" is a serious, if still debated, thread in autoimmune-disease research. Most of the strongest evidence here is still from animal models rather than controlled human trials, so treat it as an active area of research rather than settled fact.

On timing with levothyroxine: this is the one place the evidence is weaker than commonly claimed. The best controlled data available (a crossover trial, cited in a systematic review of levothyroxine-food interactions) tested grapefruit juice specifically and found only a modest, likely-clinically-small effect on absorption. The reviewers' own conclusion: people on levothyroxine shouldn't feel discouraged from normal fruit juice drinking on this basis. The interaction that IS well-established is with CALCIUM-FORTIFIED juice, not juice in general, and this app's juice list already leaves fortified products out, so that concern mostly doesn't apply to what you'd pick here.`,
...
- Whole cuts (steaks, chops, roasts) of beef, pork, lamb, or veal: 145°F, lower than for ground meat, since bacteria on an intact cut are mostly on the surface.
...
Raw fish specifically has a separate safety standard: the FDA requires fish served raw (sushi, sashimi) to first be frozen at -4°F for 7 days, or -31°F until solid then held at -31°F for 15 hours, to kill parasites like Anisakis. "Sushi-grade" isn't an official grading term, it's shorthand for fish that's gone through this freezing process, worth knowing if preparing raw fish at home rather than at a restaurant that already handles this.

This isn't a reason to avoid a rare steak or sushi-grade fish, both are common, often safe choices when handled this way. It's a reason to know the actual temperature or handling standard for whatever's being logged as raw here, especially given several conditions this app tracks involve immunosuppressive treatment (biologics, methotrexate, and similar medications), which measurably raises the infection risk from any of these foods above the general population's baseline.`,
...
const WHOLE_FOOD_MATRIX_RULE: GeneralHealthRule = {
  ...
  title: 'Whole vs. Ground Nuts and Seeds',
  message: `Grinding a whole nut or seed into flour, butter, or powder does more than change its texture: it breaks open the plant cell walls that would otherwise trap some of the fat during digestion, so a meaningful share of it can pass through less absorbed than the same weight of the whole, intact food. This is a distinct mechanism from glycemic index (it's about physical structure, not sugar), and it can cut either way depending on what's actually being tracked: less absorbed fat and calories from the ground form, or more readily available nutrients in other cases, since the same broken structure that lets fat escape also makes some nutrients easier to reach.

This isn't a reason to avoid nut butters, flours, or powders. They're useful, often more convenient forms of the same whole food. It's a reason not to assume gram-for-gram nutrition information for a whole nut and its ground form are interchangeable in practice, even when the reference data says they're similar on paper.`,
  check: (current) => {
...
const HIGH_HEAT_COOKING_RULE: GeneralHealthRule = {
  ...
  message: `Grilling, frying, broiling, and roasting meat, poultry, or fish at high, dry heat measurably increases advanced glycation end products (AGEs) compared to the same food cooked with moist heat (boiling, steaming, simmering, poaching): published data finds meat cooked this way can carry several times the AGE content of the identical food cooked more gently. AGEs are linked in the research to increased oxidative stress and inflammatory signaling.

This isn't a case for avoiding grilled or roasted food outright. It's a cited reason to notice that the cooking method itself, not just the ingredient, is part of what's actually being logged, and that moist-heat methods are a lower-AGE alternative when it matters.`,
...
const PORTION_SIZE_RULE: GeneralHealthRule = {
  ...
  message: `This entry is several times larger than this food's typical real-world serving. Larger portions and packages are well-documented (a Cochrane systematic review of 61 studies, Hollands et al. 2015) to measurably increase how much people actually eat, independent of hunger. Not a claim about this specific food being risky, just a reason a bigger-than-usual amount is worth noticing rather than logging on autopilot.`,
...
const TANNIN_IRON_RULE: GeneralHealthRule = {
  ...
  message: `Tannins in tea and coffee bind non-heme iron in the gut, reducing how much of it gets absorbed when the two are eaten close together, a well-established interaction (see this app's Nutrient Interactions research on tannins and iron). Legumes are a common plant-based iron source.

Not a reason to give up either one. Spacing tea or coffee an hour or so away from an iron-rich meal is the practical fix already established for this same interaction elsewhere.`,
