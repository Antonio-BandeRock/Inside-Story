...
// absorption by 50% or more") did not hold up under verification: the
// research here is mixed (some studies show ~28-40% less free
...
export const COFFEE_ADVISORY_MESSAGE = `If you take levothyroxine, coffee timing matters more than roast or bean choice. A pharmacokinetic study found espresso taken together with a levothyroxine dose delayed peak absorption by 38-43 minutes and reduced it by 19-36% (Benvenga et al., cited in a systematic review of levothyroxine/food interactions, PMC8002057). Coffee polyphenols and tannins can bind the medication before it's absorbed, and caffeine speeds gut transit, giving it less time to dissolve. Coffee taken about an hour after a dose showed no effect. A separate study found liquid levothyroxine solution wasn't affected even 5 minutes after coffee (Endocrine Society, 2022), worth asking your doctor about if timing is a recurring problem for you.
...
// the CATEGORY_OVERRIDES in scripts/build_food_reference_db.py). Brewing
// also holds tea/cocoa/malt items, so a coffee-specific keyword check on
// the food's base name is what narrows it down there.
