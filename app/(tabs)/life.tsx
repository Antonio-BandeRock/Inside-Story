      ...
      <SwipeableTabScreen enabled={!revealed}>
        <GatedTabContent pageTitle="Life" variant="field" revealed={revealed}>
          <ScrollView ref={scrollRef} style={styles.scroll} contentContainerStyle={[styles.content, { paddingBottom: scrollBottomPadding }]}>
            ...
            {lens === 'finances' ? (
            <View style={band.column}>
            <View style={[band.inset, styles.sectionPillRow]}>
              {SECTIONS.map((entry) => (
                <TouchableOpacity
                  key={entry.key}
            ...
            {loading ? (
              <View style={band.boxMuted}><Text style={styles.bodyText}>Adding things up…</Text></View>
            ) : section === 'overview' ? (
              renderOverview()
            ) : section === 'health' ? (
...
const styles = StyleSheet.create({
  ...
  sectionPillRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  pill: {
