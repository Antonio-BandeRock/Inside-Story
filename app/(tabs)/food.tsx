...
const FOOD_LENS_COPY: Record<FoodLens, string> = {
  mealBuilder:
    "Built 2026-08-02, last of the ten. Name the meal (optional) and choose a meal type, then \"Add from...\" pulls in one or more already-saved sides/salads/smoothies/fermentations/beverages/snacks/baked goods/soups/sauces, then pick how much of each you actually had, and repeat for as many items as the meal actually has. Checks for raw goitrogenic foods combined ACROSS the whole meal (not just within one builder's own ingredient list) before logging. Log This Now saves a meal for right now. Reconnects Schedule's/Home's own \"Log now\" action, broken since the old all-in-one builder was deleted 2026-07-25. Building once and scheduling it for a future date/time isn't wired up yet; Schedule's own existing scheduling flow still covers that in the meantime.",
  sideBuilder:
